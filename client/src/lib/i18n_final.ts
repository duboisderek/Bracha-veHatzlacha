export const translations = {
  en: {
    // Application Name
    appName: "Bracha veHatzlacha",
    appNameShort: "Bracha",
    
    // Navigation & General
    home: "Home",
    dashboard: "Personal Area",
    chat: "Chat & Support",
    admin: "Admin Panel",
    logout: "Logout",
    login: "Login",
    loginAsClient: "Enter as Client",
    
    // Home Screen
    welcomeMessage: "Welcome to Bracha veHatzlacha Lottery",
    participateInLottery: "Participate in Lottery",
    selectNumbers: "Select Numbers",
    selectAmount: "Select Amount",
    nextDraw: "Next Draw",
    timeUntilDraw: "Time Until Draw",
    jackpotAmount: "Jackpot Amount",
    quickPick: "Quick Pick",
    mustSelect6Numbers: "Must select 6 numbers",
    currentJackpot: "Current Jackpot",
    drawNumber: "Draw #",
    participationHistory: "Participation History",
    yourNumbers: "Your Numbers",
    result: "Result",
    winnings: "Winnings",
    
    // Personal Area
    currentBalance: "Current Balance",
    topupHistory: "Top-up History",
    userStatus: "User Status",
    statusNew: "New User",
    statusActive: "Active",
    statusSilver: "Silver",
    statusGold: "Gold", 
    statusDiamond: "Diamond",
    statusVIP: "VIP",
    silverRequirement: "10 participations for Silver status",
    goldRequirement: "100 participations for Gold status", 
    diamondRequirement: "500 participations for Diamond status",
    
    // Chat & Support
    liveChat: "Live Chat",
    supportRequest: "Open Support Request",
    chatWithSupport: "Chat with Support",
    typeMessage: "Type message...",
    send: "Send",
    newSupportRequest: "New Support Request",
    describeProblem: "Describe your problem",
    
    // Notifications
    drawImminent: "Draw starting soon!",
    winningNotification: "Congratulations! You won ₪{amount}",
    claimInstructions: "Contact support to claim your prize",
    participationConfirmed: "Participation confirmed",
    insufficientBalance: "Insufficient balance",
    
    // Referral System
    referralProgram: "Referral Program",
    yourReferralLink: "Your Referral Link",
    generateQRCode: "Generate QR Code",
    referralBonus: "Referral Bonus: ₪100",
    bonusCondition: "When friend tops up ₪1000+",
    copyLink: "Copy Link",
    shareLink: "Share Link",
    
    // Winners Display
    recentWinners: "Recent Winners",
    winnersCarousel: "Recent Winners",
    congratulations: "Congratulations",
    winner: "Winner",
    amount: "Amount",
    
    // Quick Contact
    contactWhatsApp: "WhatsApp",
    contactTelegram: "Telegram", 
    contactChat: "Live Chat",
    
    // Admin Panel
    adminPanel: "Admin Panel",
    platformManagement: "Bracha veHatzlacha Platform Management",
    userManagement: "User Management",
    createUser: "Create User",
    manualDeposit: "Manual Deposit",
    systemControls: "System Controls",
    currentDraw: "Current Draw",
    runDraw: "Run Draw",
    drawCompleted: "Draw Completed",
    totalUsers: "Total Users",
    totalDraws: "Total Draws",
    usersList: "Users List",
    name: "Name",
    email: "Email",
    balance: "Balance",
    status: "Status",
    actions: "Actions",
    active: "Active",
    blocked: "Blocked",
    block: "Block",
    unblock: "Unblock",
    loading: "Loading...",
    
    // Admin Messages
    userCreated: "User created successfully",
    depositSuccess: "Deposit completed successfully",
    userBlocked: "User blocked successfully",
    userUnblocked: "User unblocked successfully",
    errorLoading: "Loading error",
    errorCreating: "Error creating user",
    errorDeposit: "Error processing deposit",
    errorBlocking: "Error blocking user",
    errorDraw: "Error running draw",
    connectionError: "Connection error",
    
    // Admin Actions
    deposit: "Deposit",
    username: "Username",
    selectUser: "Select User",
    comment: "Comment",
    firstName: "First Name",
    lastName: "Last Name",
    password: "Password",
    create: "Create",
    save: "Save",
    cancel: "Cancel",
    edit: "Edit",
    delete: "Delete",
    confirm: "Confirm",
    
    // Validation & Errors
    required: "Required field",
    invalidInput: "Invalid input",
    error: "Error",
    success: "Success",
    loginRequired: "Login required",
    
    // Time & Dates
    days: "days",
    hours: "hours", 
    minutes: "minutes",
    seconds: "seconds",
    
    // Currency
    currency: "₪",
    shekel: "Shekel",
    
    // Actions
    participate: "Participate",
    topUp: "Top Up",
    claim: "Claim Prize",
    contact: "Contact",
    share: "Share",
    copy: "Copy",
    copied: "Copied!",
    
    // User Status Levels
    userLevel: "User Level",
    levelProgress: "Level Progress",
    nextLevelRequirement: "Next Level Requirement",
    
    // Previous Numbers History
    previousDrawNumbers: "Previous Draw Numbers",
    usePreviousDrawNumbers: "Use Previous Numbers",
    selectFromDrawHistory: "Select from Draw History",
    drawHistoryList: "Draw History",
    pastWinningNumbers: "Past Winning Numbers",
    
    // Contact Widgets  
    quickContactWidget: "Quick Contact",
    whatsappContact: "WhatsApp",
    telegramContact: "Telegram",
    liveChatContact: "Live Chat",
    
    // Draw Lock System
    drawLockWarning: "Draw starts soon",
    lockMessage: "Participation locked 60 seconds before draw",
    remainingTime: "Time remaining",
    
    // Notification System
    notificationSettings: "Notification Settings",
    drawStartNotification: "Draw is starting soon!",
    winningNotificationText: "Congratulations! You won",
    prizeClaimInstruction: "Claim your prize",
    
    // Referral Features
    referralSystem: "Referral System",
    myReferralCode: "My Referral Code",
    referralEarnings: "Referral Earnings",
    depositBonus: "100₪ bonus when referred user makes first deposit of 1000₪+",
    createQRCode: "Create QR Code",
    shareMyReferralLink: "Share My Referral Link",
    
    // Missing translations for NumberSelection
    selectYourLuckyNumbers: "Select Your Lucky Numbers",
    chooseNumbers: "Choose 6 numbers from 1 to 37",
    
    // Login/Auth Pages
    clientLogin: "Client Login",
    clientLoginWelcome: "Welcome to BrachaVeHatzlacha",
    loginToAccount: "Sign in to your account",
    enterEmail: "Enter your email",
    enterPassword: "Enter your password",
    signIn: "Sign In",
    signingIn: "Signing in...",
    loginError: "Login failed. Please try again.",
    tryDemo: "Try Demo Account",
    noAccount: "Don't have an account?",
    registerHere: "Register here",
    backToHome: "Back to Home",
    or: "or",
    adminLogin: "Admin Login",
    adminLoginWelcome: "Administrative Access",
    adminLoginDescription: "Access restricted to administrators",
    adminSignIn: "Sign In as Admin",
    adminCredentials: "Admin Credentials",
    adminAccess: "Admin access?",
    clientAccess: "Client access?",
    welcomeToLottery: "Welcome to Lottery",
    register: "Register",
    phoneNumber: "Phone Number",
    confirmPassword: "Confirm Password", 
    createAccount: "Create Account",
    
    // Missing Home Page translations
    lastUpdated: "Last Updated",
    reuseNumbers: "Reuse Numbers",
    participationAmount: "Participation Amount",
    participationLocked: "Participation Locked",
    nextDrawInfo: "Next Draw Info",
    drawCountdown: "Draw Countdown",
    standardLotterySuggestion: "Standard Lottery Suggestion",
    
    // Missing Admin translations
    drawCreated: "Draw created successfully",
    resultsSubmitted: "Results submitted successfully",
    createUserButton: "Create User",
    drawDate: "Draw Date",
    yourSelectedNumbers: "Your Selected Numbers",
    selected: "Selected",
    costPerTicket: "Cost per Ticket",
    purchaseTicket: "Purchase Ticket",
    previousNumbers: "Previous Numbers",
    manualSelection: "Manual Selection",
    fromHistory: "From History",
    
    // User Level Progression
    currentLevel: "Current Level",
    progressToNext: "Progress to Next Level",
    participationsLeft: "participations needed",
    levelUpgrade: "Level Upgrade!",
    
    // Dashboard Actions
    quickActions: "Quick Actions",
    addFunds: "Add Funds",
    myHistory: "My History",
    supportChat: "Support Chat",
    lastDrawResults: "Last Draw Results",
    draw: "Draw",
    matches: "matches",
    winners: "winners",
    each: "each",
    myActiveTickets: "My Active Tickets",
    
    // Previous Numbers Selector
    hotNumbers: "Hot Numbers",
    coldNumbers: "Cold Numbers",
    useTheseNumbers: "Use These Numbers",
    recentDraws: "Recent Draws",
    
    // New MVP Features
    chatSupport: "Chat Support",
    referFriends: "Refer Friends",
    referralDescription: "Invite friends and earn 100₪ when they make their first deposit of 1000₪+",
    progressToBonus: "Progress to Bonus",
    referralsCount: "Referrals Count",
    won: "Won",
    correct_numbers: "Correct Numbers",
    scan_qr_referral: "Scan QR code to join with your referral link",
    draw_locked: "Draw Locked",
    participation_locked: "New participations are locked 60 seconds before draw",
    standard_lottery: "Standard Lottery",
    join_standard_lottery: "Join Standard Lottery",
    jackpot_updated: "Jackpot Updated",
    last_update: "Last Update",
    auto_update_in: "Auto update in",
    
    // Landing Page
    landingWelcome: "Welcome to the Ultimate Lottery Experience",
    landingSubtitle: "Join thousands of players worldwide",
    currentJackpotLabel: "Current Jackpot",
    nextDrawLabel: "Next Draw",
    drawsEveryWeek: "Draws Every Week",
    fridayAt8PM: "Friday at 8:00 PM",
    joinNow: "Join Now",
    loginAsAdmin: "Admin Access",
    demoAccess: "Demo Access",
    chooseYourRole: "Choose Your Access",
    
    // Features
    instantPayouts: "Instant Payouts",
    multiLanguage: "Multi-Language",
    
    // Stats
    totalPlayers: "Total Players",
    weeklyJackpot: "Weekly Jackpot",
    winnersToDate: "Winners to Date",
    
    // Auth Forms
    enterCredentials: "Enter Your Credentials",
    emailAddress: "Email Address",
    passwordField: "Password",
    loginButton: "Login",
    registerButton: "Register",
    alreadyHaveAccount: "Already have an account?",
    dontHaveAccount: "Don't have an account?",
    
    // Admin Interface
    adminDashboard: "Admin Dashboard",
    managePlatform: "Manage Platform Operations",
    totalTickets: "Total Tickets",
    totalRevenue: "Total Revenue",
    activeUsers: "Active Users",
    completedDraws: "Completed Draws",
    
    // Demo Messages
    demoMode: "Demo Mode",
    demoModeDescription: "This is a demonstration with sample data",
    
    // Error Messages
    loginFailed: "Login failed. Please try again.",
    networkError: "Network error. Please check your connection.",
    unexpectedError: "An unexpected error occurred.",
    
    // Success Messages
    loginSuccess: "Login successful",
    accountCreated: "Account created successfully",
    passwordChanged: "Password changed successfully",
    
    // Navigation
    backToLogin: "Back to Login",
    goToHome: "Go to Home",
    viewProfile: "View Profile",
    accountSettings: "Account Settings",
    
    // Admin Dashboard Missing Keys
    jackpot: "Jackpot",
    prizePool: "Prize Pool",
    revenue: "Revenue",
    ticketSales: "Ticket Sales",
    totalWinners: "Total Winners",
    submitDrawResults: "Submit Draw Results",
    date: "Date",
    selectWinningNumbers: "Select Winning Numbers",
    winningNumbers: "Winning Numbers",
    processing: "Processing",
    submitResults: "Submit Results",
    drawStatistics: "Draw Statistics",
    ticketsText: "tickets",
    prizeDistribution: "Prize Distribution",
    platformRetention: "Platform Retention",
    user: "User",
    drawCreatedSuccessfully: "Draw created successfully",
    selectDrawDate: "Select draw date",
    createNewDraw: "Create New Draw",
    creating: "Creating",
    createDraw: "Create Draw",
    platformRevenue: "Platform Revenue",
    retentionFromDraw: "Retention from draw",
    depositSuccessful: "Deposit successful",
    invalidAmount: "Invalid amount",
    searchUserByEmail: "Search user by email",
    userNotFound: "User not found",
    depositAmount: "Deposit Amount",
    enterAmount: "Enter amount",
    enterComment: "Enter comment (optional)",
    depositing: "Depositing",
    yourWinningNumbers: "Your Winning Numbers",
    noCompletedDraws: "No completed draws found",
    
    // LANDING PAGE MISSING TRANSLATIONS - ENGLISH
    massiveJackpots: "Massive Jackpots",
    lifeChangingPrizes: "Life-changing prizes every draw",
    advancedSecurityPayouts: "Advanced security & instant payouts",
    referralRewards: "Referral Rewards",
    earnForReferrals: "Earn ₪100 for each friend you refer",
    multipleChancesToWin: "Multiple chances to win every week",
    readyToChangeLife: "Ready to Change Your Life?",
    joinTodayAndStart: "Join Bracha veHatzlacha today and start your journey to fortune",
    startPlayingNow: "Start Playing Now",
    weeklyDraws: "Weekly Draws",
    secureAndTrusted: "100% Secure",
    
    // FINAL ENGLISH MISSING TRANSLATIONS
    fraudPrevention: "Fraud Prevention",
    gameHistory: "Game History",
    gameNotifications: "Game Notifications",
    help: "Help",
    helpCenterMenu: "Help Center Menu",
    identityVerification: "Identity Verification",
    inactiveStatus: "Inactive Status",
    infoNotifications: "Info Notifications",
    integrations: "Integrations",
    ipBlacklist: "IP Blacklist",
    jackpotNotifications: "Jackpot Notifications",
    kycVerification: "KYC Verification",
    loyaltyProgram: "Loyalty Program",
    menu: "Menu",
    mobileApp: "Mobile App",
    multiCurrency: "Multi Currency",
    notifications: "Notifications",
    onlineStatus: "Online Status",
    paymentGateway: "Payment Gateway",
    pendingDraw: "Pending Draw",
    personalInfo: "Personal Info",
    playerRanking: "Player Ranking",
    policy: "Policy",
    preferences: "Preferences",
    privacy: "Privacy",
    profile: "Profile",
    promotions: "Promotions",
    quickSupport: "Quick Support",
    realTimeUpdates: "Real Time Updates",
    referralProgram: "Referral Program",
    reportIssue: "Report Issue",
    securityAlerts: "Security Alerts",
    smartphoneApp: "Smartphone App",
    systemMaintenance: "System Maintenance",
    terms: "Terms",
    thirdPartyIntegrations: "Third Party Integrations",
    transactionHistory: "Transaction History",
    userFeedback: "User Feedback",
    userPreferences: "User Preferences",
    userSettings: "User Settings",
    versionUpdate: "Version Update",
    webNotifications: "Web Notifications",
    welcomePackage: "Welcome Package",
    winNotifications: "Win Notifications",

    // ===== ROLE-BASED ACCESS TRANSLATIONS =====
    
    // ROOT ADMIN ACCESS
    rootAdminPanel: "Root Admin Panel",
    rootAdminAccess: "Root Admin Access",
    systemManagement: "System Management",
    globalSettings: "Global Settings",
    userRoleManagement: "User Role Management",
    systemSecurity: "System Security",
    databaseManagement: "Database Management",
    backupAndRestore: "Backup & Restore",
    systemLogs: "System Logs",
    globalStatistics: "Global Statistics",
    platformConfiguration: "Platform Configuration",
    rootAdminDashboard: "Root Admin Dashboard",
    manageAdmins: "Manage Admins",
    createAdmin: "Create Admin",
    deleteAdmin: "Delete Admin",
    adminPermissions: "Admin Permissions",
    systemHealth: "System Health",
    serverStatus: "Server Status",
    maintenanceMode: "Maintenance Mode",
    criticalSettings: "Critical Settings",
    
    // STANDARD ADMIN ROLE
    standardAdminPanel: "Standard Admin Panel",
    standardAdminAccess: "Standard Admin Access",
    clientManagement: "Client Management",
    drawManagement: "Draw Management",
    transactionMonitoring: "Transaction Monitoring",
    reportGeneration: "Report Generation",
    supportManagement: "Support Management",
    manageClients: "Manage Clients",
    createClient: "Create Client",
    blockClient: "Block Client",
    unblockClient: "Unblock Client",
    viewClientHistory: "View Client History",
    clientTransactions: "Client Transactions",
    manualRefund: "Manual Refund",
    clientSupport: "Client Support",
    messagingSystem: "Messaging System",
    bulkOperations: "Bulk Operations",
    
    // VIP CLIENT ROLE
    vipClientPanel: "VIP Client Panel",
    vipClientAccess: "VIP Client Access",
    vipBenefits: "VIP Benefits",
    prioritySupport: "Priority Support",
    exclusiveDraws: "Exclusive Draws",
    higherLimits: "Higher Limits",
    personalAccount: "Personal Account Manager",
    vipRewards: "VIP Rewards",
    exclusiveOffers: "Exclusive Offers",
    fastWithdrawals: "Fast Withdrawals",
    vipChat: "VIP Chat",
    dedicatedSupport: "Dedicated Support",
    vipHistory: "VIP History",
    bonusProgram: "Bonus Program",
    loyaltyPoints: "Loyalty Points",
    specialEvents: "Special Events",
    
    // STANDARD CLIENT ROLE
    standardClientPanel: "Standard Client Panel",
    standardClientAccess: "Standard Client Access",
    basicFeatures: "Basic Features",
    standardSupport: "Standard Support",
    regularDraws: "Regular Draws",
    standardLimits: "Standard Limits",
    myAccount: "My Account",
    playHistory: "Play History",
    winningsHistory: "Winnings History",
    accountBalance: "Account Balance",
    depositFunds: "Deposit Funds",
    withdrawFunds: "Withdraw Funds",
    myTickets: "My Tickets",
    upcomingDraws: "Upcoming Draws",
    pastResults: "Past Results",
    
    // NEW CLIENT ROLE
    newClientPanel: "New Client Panel",
    newClientAccess: "New Client Access",
    welcomeBonus: "Welcome Bonus",
    tutorial: "Tutorial",
    gettingStarted: "Getting Started",
    firstDeposit: "First Deposit",
    learningMode: "Learning Mode",
    helpGuide: "Help Guide",
    basicInstructions: "Basic Instructions",
    accountSetup: "Account Setup",
    profileCompletion: "Profile Completion",
    verificationProcess: "Verification Process",
    welcomeOffer: "Welcome Offer",
    trialPeriod: "Trial Period",
    introductoryLimits: "Introductory Limits",
    
    // STATES AND STATUSES
    enabled: "Enabled",
    disabled: "Disabled",
    available: "Available",
    unavailable: "Unavailable",
    online: "Online",
    offline: "Offline",
    pending: "Pending",
    completed: "Completed",
    failed: "Failed",
    cancelled: "Cancelled",
    
    // ADDITIONAL SYSTEM FEATURES
    about: "About",
    account: "Account",
    accountActivation: "Account Activation",
    accountIssues: "Account Issues",
    accountLockout: "Account Lockout",
    accountSecurity: "Account Security",
    accountSettingsMenu: "Account Settings",
    accountVerification: "Account Verification",
    activeDraw: "Active Draw",
    activeStatus: "Active Status",
    advancedStats: "Advanced Stats",
    alertsManagement: "Alerts Management",
    analytics: "Analytics",
    apiManagement: "API Management",
    auditLogs: "Audit Logs",
    autoResponder: "Auto Responder",
    autoVerification: "Auto Verification",
    bankTransfer: "Bank Transfer",
    bannedStatus: "Banned Status",
    betLimits: "Bet Limits",
    bitcoinPayment: "Bitcoin Payment",
    bounceRate: "Bounce Rate",
    browserNotifications: "Browser Notifications",
    bulkEmail: "Bulk Email",
    bulkSMS: "Bulk SMS",
    cancelledDraw: "Cancelled Draw",
    cancelledTransaction: "Cancelled Transaction",
    cardPayment: "Card Payment",
    chargebackProtection: "Chargeback Protection",
    churnAnalysis: "Churn Analysis",
    claimBonus: "Claim Bonus",
    clickRate: "Click Rate",
    cohortAnalysis: "Cohort Analysis",
    complaintSubmission: "Complaint Submission",
    completedDraw: "Completed Draw",
    completedTransaction: "Completed Transaction",
    confirmedCrypto: "Confirmed Crypto",
    contactInfo: "Contact Info",
    contactSupport: "Contact Support",
    contentManagement: "Content Management",
    conversionAnalytics: "Conversion Analytics",
    cooloffPeriod: "Cooloff Period",
    cryptoDeposit: "Crypto Deposit",
    cryptoFees: "Crypto Fees",
    cryptoHistory: "Crypto History",
    cryptoPayments: "Crypto Payments",
    cryptoSettings: "Crypto Settings",
    cryptoSupport: "Crypto Support",
    cryptoWallet: "Crypto Wallet",
    cryptoWithdrawal: "Crypto Withdrawal",
    currencySettings: "Currency Settings",
    customLimits: "Custom Limits",
    customerSupport: "Customer Support",
    customerSupportMain: "Customer Support",
    deleteAdmin: "Delete Admin",
    deviceManagement: "Device Management",
    digitalWallet: "Digital Wallet",
    disableTwoFA: "Disable 2FA",
    documentUpload: "Document Upload",
    drawAnalytics: "Draw Analytics",
    drawSchedule: "Draw Schedule",
    drawStates: "Draw States",
    emailConfig: "Email Config",
    emailDelivery: "Email Delivery",
    emailHistory: "Email History",
    emailNotifications: "Email Notifications",
    emailQueue: "Email Queue",
    emailSupport: "Email Support",
    emailSystem: "Email System",
    emailTemplates: "Email Templates",
    emailValidation: "Email Validation",
    enableTwoFA: "Enable 2FA",
    errorLogs: "Error Logs",
    eventTracking: "Event Tracking",
    excludedUser: "Excluded User",
    expiredDraw: "Expired Draw",
    exportData: "Export Data",
    faqSection: "FAQ Section",
    feedbackForm: "Feedback Form",
    fraudDetection: "Fraud Detection",
    gameConfiguration: "Game Configuration",
    gameIssues: "Game Issues",
    gameSettings: "Game Settings",
    generalInquiry: "General Inquiry",
    generalSettings: "General Settings",
    geoBlocking: "Geo Blocking",
    helpCenterMain: "Help Center",
    historyTracking: "History Tracking",
    inAppNotifications: "In-App Notifications",
    incompleteTransaction: "Incomplete Transaction",
    ipRestrictions: "IP Restrictions",
    knowledgeBase: "Knowledge Base",
    languageSettings: "Language Settings",
    liveChatSupport: "Live Chat Support",
    loginAttempts: "Login Attempts",
    lotteryOperations: "Lottery Operations",
    maintenanceNotification: "Maintenance Notification",
    marketingCampaigns: "Marketing Campaigns",
    multipleDevices: "Multiple Devices",
    networkIssues: "Network Issues",
    notificationChannels: "Notification Channels",
    operationalLogs: "Operational Logs",
    passwordPolicy: "Password Policy",
    paymentIssues: "Payment Issues",
    paymentMethods: "Payment Methods",
    pendingTransaction: "Pending Transaction",
    personalizedOffers: "Personalized Offers",
    phoneSupport: "Phone Support",
    playTime: "Play Time",
    prioritySupportService: "Priority Support",
    privacySettings: "Privacy Settings",
    pushNotifications: "Push Notifications",
    realityCheck: "Reality Check",
    responsibleGaming: "Responsible Gaming",
    riskAssessment: "Risk Assessment",
    searchHelp: "Search Help",
    securityConfiguration: "Security Configuration",
    securityQuestions: "Security Questions",
    selfExclusion: "Self Exclusion",
    sessionTimeout: "Session Timeout",
    smsAlerts: "SMS Alerts",
    spendingLimits: "Spending Limits",
    stepByStepGuides: "Step by Step Guides",
    suggestionBox: "Suggestion Box",
    supportHours: "Support Hours",
    supportLanguages: "Support Languages",
    systemHealth: "System Health",
    telegramNotifications: "Telegram Notifications",
    themeSettings: "Theme Settings",
    ticketSupport: "Ticket Support",
    timezoneSettings: "Timezone Settings",
    transactionLimits: "Transaction Limits",
    troubleshooting: "Troubleshooting",
    tutorials: "Tutorials",
    twoFactorAuthentication: "Two Factor Authentication",
    videoGuides: "Video Guides",
    whatsappNotifications: "WhatsApp Notifications",
    suspendedUser: "Suspended User",
    verificationRequired: "Verification Required",
    withdrawalLimits: "Withdrawal Limits",
    requestTypes: "Request Types",
    technicalSupport: "Technical Support",
    displaySettings: "Display Settings",
    accessibilitySettings: "Accessibility Settings",
    
    // AUTHENTICATION ADDITIONS
    adminCredentials: "Admin Credentials",
    adminLogin: "Admin Login",
    adminLoginDescription: "Enter your admin credentials to access the management panel",
    adminLoginWelcome: "Welcome to Admin Portal",
    adminAccess: "Admin Access",
    adminSignIn: "Admin Sign In",
    clientLoginWelcome: "Welcome to Client Portal",
    enterEmail: "Enter Email",
    enterPassword: "Enter Password",
    loginError: "Login Error",
    authenticationFailed: "Authentication Failed",
    secureLogin: "Secure Login",
    verifyCredentials: "Verify Credentials",
    accessDenied: "Access Denied",
    invalidCredentials: "Invalid Credentials",
    passwordRequired: "Password Required",
    emailRequired: "Email Required",
    registrationSuccessful: "Registration Successful",
    finalCleanTranslations: "Final Clean Translations",
    
    // REMAINING MISSING TRANSLATIONS
    backupCodes: "Backup Codes",
    basicSettings: "Basic Settings",
    clientOverview: "Client Overview",
    ethereumPayment: "Ethereum Payment",
    exchangeRate: "Exchange Rate",
    exclusiveGames: "Exclusive Games",
    expired: "Expired",
    failedTransaction: "Failed Transaction",
    finalHebrew: "Perfect Hebrew",
    firstSteps: "First Steps",
    fraudReport: "Fraud Report",
    gameRules: "Game Rules",
    highRisk: "High Risk",
    insufficientFunds: "Insufficient Funds",
    inviteBonus: "Invite Bonus",
    lastSeen: "Last Seen",
    litecoinPayment: "Litecoin Payment",
    lowRisk: "Low Risk",
    memberSince: "Member Since",
    moderateRisk: "Moderate Risk",
    newUser: "New User",
    onHold: "On Hold",
    overdue: "Overdue",
    paymentConfirmation: "Payment Confirmation",
    paymentHistory: "Payment History",
    phoneNumber: "Phone Number",
    premiumFeatures: "Premium Features",
    processingError: "Processing Error",
    quickActions: "Quick Actions",
    raceToWin: "Race to Win",
    refunded: "Refunded",
    resendCode: "Resend Code",
    retryPayment: "Retry Payment",
    securityEvent: "Security Event",
    sendMessage: "Send Message",
    settingsPanel: "Settings Panel",
    smartContract: "Smart Contract",
    socialMedia: "Social Media",
    successMessage: "Success Message",
    supportTeam: "Support Team",
    taxInfo: "Tax Info",
    temporaryBan: "Temporary Ban",
    unconfirmed: "Unconfirmed",
    upcomingFeatures: "Upcoming Features",
    userActivity: "User Activity",
    verification: "Verification",
    viewDetails: "View Details",
    walletAddress: "Wallet Address",
    weeklyReport: "Weekly Report",
    withdrawalError: "Withdrawal Error",
    yearToDate: "Year to Date",
    
    // ===== FINAL MISSING ENGLISH TRANSLATIONS FOR 100% COVERAGE =====
    dashboardActions: "Dashboard Actions",
    learningCenter: "Learning Center",
    lockedDraw: "Locked Draw",
    loginHistory: "Login History",
    loginToAccount: "Login to Account",
    manualReview: "Manual Review",
    maximumWithdrawal: "Maximum Withdrawal",
    minimumWithdrawal: "Minimum Withdrawal",
    moderationTools: "Moderation Tools",
    monitoringDashboard: "Monitoring Dashboard",
    myDashboard: "My Dashboard",
    myTransactions: "My Transactions",
    noAccount: "No Account",
    notificationCenter: "Notification Center",
    onboardingGuide: "Onboarding Guide",
    operationalDashboard: "Operational Dashboard",
    or: "or",
    paymentProcessor: "Payment Processor",
    permissionControl: "Permission Control",
    playerStatistics: "Player Statistics",
    qualityAssurance: "Quality Assurance",
    reconciliationReport: "Reconciliation Report",
    recurringPayment: "Recurring Payment",
    referralTracking: "Referral Tracking",
    reportDownload: "Report Download",
    revenueStream: "Revenue Stream",
    riskManagement: "Risk Management",
    schedulePayment: "Schedule Payment",
    securityProtocol: "Security Protocol",
    serviceStatus: "Service Status",
    stakeholderReport: "Stakeholder Report",
    systemAlert: "System Alert",
    timelineView: "Timeline View",
    transactionFee: "Transaction Fee",
    userEngagement: "User Engagement",
    validationRule: "Validation Rule",
    workflowAutomation: "Workflow Automation",
    
    // ===== FINAL ADDITIONAL ENGLISH TRANSLATIONS FOR 100% COVERAGE =====
    notificationTypes: "Notification Types",
    openRate: "Open Rate",
    passwordChange: "Password Change",
    paymentNotifications: "Payment Notifications",
    paymentProcess: "Payment Process",
    paymentProcessing: "Payment Processing",
    paymentSecurity: "Payment Security",
    pendingCrypto: "Pending Crypto",
    pendingWithdrawals: "Pending Withdrawals",
    performanceMetrics: "Performance Metrics",
    personalizedSupport: "Personalized Support",
    platformAnalytics: "Platform Analytics",
    platformMaintenance: "Platform Maintenance",
    playerRewards: "Player Rewards",
    priorityAccess: "Priority Access",
    quickWithdrawal: "Quick Withdrawal",
    realTimeSupport: "Real Time Support",
    recentActivity: "Recent Activity",
    referralBonus: "Referral Bonus",
    registerHere: "Register Here",
    reportGeneration: "Report Generation",
    revenueTracking: "Revenue Tracking",
    riskMonitoring: "Risk Monitoring",
    secureAndTrusted: "Secure and Trusted",
    securityEvents: "Security Events",
    signIn: "Sign In",
    smartphoneApp: "Smartphone App",
    socialSharing: "Social Sharing",
    specialOffers: "Special Offers",
    systemOverview: "System Overview",
    thirdPartyIntegrations: "Third Party Integrations",
    transactionAnalytics: "Transaction Analytics",
    transactionHistory: "Transaction History",
    userAcquisition: "User Acquisition",
    userFeedback: "User Feedback",
    userPreferences: "User Preferences",
    userRetention: "User Retention",
    userSettings: "User Settings",
    versionUpdate: "Version Update",
    webNotifications: "Web Notifications",
    welcomePackage: "Welcome Package",
    winNotifications: "Win Notifications",
    massiveJackpots: "Massive Jackpots",
    lifeChangingPrizes: "Life Changing Prizes",
    advancedSecurityPayouts: "Advanced Security and Payouts",
    startPlayingNow: "Start Playing Now",
    weeklyDraws: "Weekly Draws",
    referralRewards: "Referral Rewards",
    earnForReferrals: "Earn for Referrals",
    multipleChancesToWin: "Multiple Chances to Win",
    readyToChangeLife: "Ready to Change Life",
    joinTodayAndStart: "Join Today and Start"
  },
  
  he: {
    // Application Name
    appName: "ברכה והצלחה",
    appNameShort: "ברכה",
    
    // Navigation & General
    home: "בית",
    dashboard: "אזור אישי",
    chat: "צ'אט ותמיכה",
    admin: "פאנל ניהול",
    logout: "התנתק",
    login: "התחבר",
    loginAsClient: "כניסה כלקוח",
    
    // Home Screen
    welcomeMessage: "ברוכים הבאים ללוטו ברכה והצלחה",
    participateInLottery: "השתתפות בלוטו",
    selectNumbers: "בחירת מספרים",
    selectAmount: "בחירת סכום",
    nextDraw: "הגרלה הבאה",
    timeUntilDraw: "זמן עד ההגרלה",
    jackpotAmount: "סכום הג'קפוט",
    quickPick: "בחירה מהירה",
    mustSelect6Numbers: "יש לבחור 6 מספרים",
    currentJackpot: "הג'קפוט הנוכחי",
    drawNumber: "הגרלה מס'",
    participationHistory: "היסטוריית השתתפות",
    yourNumbers: "המספרים שלך",
    result: "תוצאה",
    winnings: "זכיות",
    
    // Personal Area
    currentBalance: "יתרה נוכחית",
    topupHistory: "היסטוריית טעינות",
    userStatus: "סטטוס משתמש",
    statusNew: "משתמש חדש",
    statusActive: "פעיל",
    statusSilver: "כסף",
    statusGold: "זהב",
    statusDiamond: "יהלום",
    statusVIP: "VIP",
    silverRequirement: "10 השתתפויות לסטטוס כסף",
    goldRequirement: "100 השתתפויות לסטטוס זהב",
    diamondRequirement: "500 השתתפויות לסטטוס יהלום",
    
    // Chat & Support
    liveChat: "צ'אט חי",
    supportRequest: "פתיחת פנייה לתמיכה",
    chatWithSupport: "צ'אט עם התמיכה",
    typeMessage: "הקלד הודעה...",
    send: "שלח",
    newSupportRequest: "פנייה חדשה לתמיכה",
    describeProblem: "תאר את הבעיה",
    
    // Notifications
    drawImminent: "ההגרלה מתחילה בקרוב!",
    winningNotification: "מזל טוב! זכית ב-₪{amount}",
    claimInstructions: "צור קשר עם התמיכה לקבלת הפרס",
    participationConfirmed: "השתתפות אושרה",
    insufficientBalance: "יתרה לא מספיקה",
    
    // Referral System
    referralProgram: "תוכנית הפניות",
    yourReferralLink: "קישור ההפניה שלך",
    generateQRCode: "יצירת קוד QR",
    referralBonus: "בונוס הפניה: ₪100",
    bonusCondition: "כאשר חבר טוען ₪1000+",
    copyLink: "העתק קישור",
    shareLink: "שתף קישור",
    
    // Winners Display
    recentWinners: "זוכים אחרונים",
    winnersCarousel: "זוכים אחרונים",
    congratulations: "מזל טוב",
    winner: "זוכה",
    amount: "סכום",
    
    // Quick Contact
    contactWhatsApp: "ווטסאפ",
    contactTelegram: "טלגרם",
    contactChat: "צ'אט חי",
    whatsapp_support: "תמיכה בווטסאפ",
    whatsapp_support_tooltip: "צור קשר איתנו בווטסאפ",
    whatsapp_default_message: "שלום! אני זקוק לעזרה עם האתר ברכה והצלחה",
    
    // Admin Panel
    adminPanel: "פאנל ניהול",
    platformManagement: "ניהול פלטפורמת ברכה והצלחה",
    userManagement: "ניהול משתמשים",
    createUser: "יצירת משתמש",
    manualDeposit: "הפקדה ידנית",
    systemControls: "בקרות מערכת",
    currentDraw: "הגרלה נוכחית",
    runDraw: "הפעלת הגרלה",
    drawCompleted: "הגרלה הושלמה",
    totalUsers: "סך משתמשים",
    totalDraws: "סך הגרלות",
    usersList: "רשימת משתמשים",
    name: "שם",
    email: "אימייל",
    balance: "יתרה",
    status: "סטטוס",
    actions: "פעולות",
    active: "פעיל",
    blocked: "חסום",
    block: "חסום",
    unblock: "בטל חסימה",
    loading: "טוען...",
    
    // Admin Messages
    userCreated: "משתמש נוצר בהצלחה",
    depositSuccess: "הפקדה הושלמה בהצלחה",
    userBlocked: "משתמש נחסם בהצלחה",
    userUnblocked: "חסימת משתמש בוטלה בהצלחה",
    errorLoading: "שגיאה בטעינה",
    errorCreating: "שגיאה ביצירת משתמש",
    errorDeposit: "שגיאה בעיבוד הפקדה",
    errorBlocking: "שגיאה בחסימת משתמש",
    errorDraw: "שגיאה בהפעלת הגרלה",
    connectionError: "שגיאת חיבור",
    
    // Admin Actions
    deposit: "הפקדה",
    username: "שם משתמש",
    selectUser: "בחר משתמש",
    comment: "הערה",
    firstName: "שם פרטי",
    lastName: "שם משפחה",
    password: "סיסמה",
    create: "צור",
    save: "שמור",
    cancel: "בטל",
    edit: "ערוך",
    delete: "מחק",
    confirm: "אשר",
    
    // Validation & Errors
    required: "שדה חובה",
    invalidInput: "קלט לא תקין",
    error: "שגיאה",
    success: "הצלחה",
    loginRequired: "נדרשת התחברות",
    
    // Time & Dates
    days: "ימים",
    hours: "שעות",
    minutes: "דקות",
    seconds: "שניות",
    
    // Currency
    currency: "₪",
    shekel: "שקל",
    
    // Actions
    participate: "השתתף",
    topUp: "טען יתרה",
    claim: "קבל פרס",
    contact: "צור קשר",
    share: "שתף",
    copy: "העתק",
    copied: "הועתק!",
    
    // User Status Levels Hebrew
    userLevel: "רמת משתמש",
    levelProgress: "התקדמות ברמה",
    nextLevelRequirement: "דרישת הרמה הבאה",
    
    // Previous Numbers History Hebrew
    previousDrawNumbers: "מספרי הגרלות קודמות",
    usePreviousDrawNumbers: "השתמש במספרים קודמים",
    selectFromDrawHistory: "בחר מהיסטוריית הגרלות",
    drawHistoryList: "רשימת היסטוריית הגרלות",
    pastWinningNumbers: "מספרים זוכים מהעבר",
    
    // Contact Widgets Hebrew
    quickContactWidget: "יצירת קשר מהיר",
    whatsappContact: "ווטסאפ",
    telegramContact: "טלגרם",
    liveChatContact: "צ'אט חי",
    
    // Draw Lock System Hebrew
    drawLockWarning: "ההגרלה מתחילה בקרוב",
    lockMessage: "השתתפות נחסמה 60 שניות לפני ההגרלה",
    remainingTime: "זמן נותר",
    
    // Notification System Hebrew
    notificationSettings: "הגדרות התראות",
    drawStartNotification: "ההגרלה מתחילה בקרוב!",
    winningNotificationText: "מזל טוב! זכית",
    prizeClaimInstruction: "אסוף את הפרס שלך",
    
    // Referral Features Hebrew
    referralSystem: "מערכת הפניות",
    myReferralCode: "קוד ההפניה שלי",
    referralEarnings: "רווחי הפניות",
    depositBonus: "בונוס 100₪ כאשר המופנה מבצע הפקדה ראשונה של 1000₪+",
    createQRCode: "יצור קוד QR",
    shareMyReferralLink: "שתף את קישור ההפניה שלי",
    
    // Missing translations for NumberSelection Hebrew
    selectYourLuckyNumbers: "בחר את המספרים המזל שלך",
    chooseNumbers: "בחר 6 מספרים מ-1 עד 37",
    yourSelectedNumbers: "המספרים שבחרת",
    selected: "נבחר",
    costPerTicket: "עלות כרטיס",
    purchaseTicket: "רכישת כרטיס",
    previousNumbers: "מספרים קודמים",
    manualSelection: "בחירה ידנית",
    fromHistory: "מההיסטוריה",
    
    // User Level Progression Hebrew
    currentLevel: "רמה נוכחית",
    progressToNext: "התקדמות לרמה הבאה",
    participationsLeft: "השתתפויות נדרשות",
    levelUpgrade: "שדרוג רמה!",
    
    // Dashboard Actions Hebrew
    quickActions: "פעולות מהירות",
    addFunds: "הוספת כספים",
    myHistory: "ההיסטוריה שלי",
    supportChat: "צ'אט תמיכה",
    lastDrawResults: "תוצאות הגרלה אחרונה",
    draw: "הגרלה",
    matches: "התאמות",
    winners: "זוכים",
    each: "כל אחד",
    myActiveTickets: "הכרטיסים הפעילים שלי",
    
    // Previous Numbers Selector Hebrew
    hotNumbers: "מספרים חמים",
    coldNumbers: "מספרים קרים",
    useTheseNumbers: "השתמש במספרים אלה",
    recentDraws: "הגרלות אחרונות",
    
    // Missing Landing Page translations Hebrew
    clientLogin: "כניסת לקוחות",
    backToHome: "חזרה לבית",
    clientAccess: "גישת לקוחות",
    welcomeToLottery: "ברוכים הבאים ללוטו",
    register: "הרשמה",
    phoneNumber: "מספר טלפון",
    confirmPassword: "אישור סיסמה",
    createAccount: "יצירת חשבון",
    
    // Missing Home Page translations Hebrew
    lastUpdated: "עודכן לאחרונה",
    reuseNumbers: "שימוש חוזר במספרים",
    participationAmount: "סכום השתתפות",
    participationLocked: "השתתפות נעולה",
    nextDrawInfo: "מידע הגרלה הבאה",
    drawCountdown: "ספירה לאחור להגרלה",
    standardLotterySuggestion: "הצעת לוטו רגיל",
    
    // New MVP Features Hebrew
    chatSupport: "צ'אט תמיכה",
    referFriends: "הפנה חברים",
    referralDescription: "הזמן חברים וקבל 100₪ כשהם מבצעים הפקדה ראשונה של 1000₪+",
    progressToBonus: "התקדמות לבונוס",
    referralsCount: "מספר הפניות",
    won: "זכה",
    correct_numbers: "מספרים נכונים",
    scan_qr_referral: "סרוק QR כדי להצטרף עם קישור ההפניה שלך",
    draw_locked: "הגרלה נעולה",
    participation_locked: "השתתפות חדשה נעולה 60 שניות לפני ההגרלה",
    standard_lottery: "לוטו רגיל",
    join_standard_lottery: "הצטרף ללוטו רגיל",
    jackpot_updated: "הקופה עודכנה",
    last_update: "עדכון אחרון",
    auto_update_in: "עדכון אוטומטי תוך",
    
    // Missing Admin translations
    drawCreated: "הגרלה נוצרה בהצלחה",
    resultsSubmitted: "תוצאות נשלחו בהצלחה",
    createUserButton: "צור משתמש",
    drawDate: "תאריך הגרלה",
    
    // Landing Page Hebrew
    landingWelcome: "ברוכים הבאים לחוויית הלוטו המושלמת",
    landingSubtitle: "הצטרפו לאלפי שחקנים ברחבי העולם",
    currentJackpotLabel: "הג'קפוט הנוכחי",
    nextDrawLabel: "הגרלה הבאה",
    drawsEveryWeek: "הגרלות כל שבוע",
    fridayAt8PM: "יום שישי בשעה 20:00",
    joinNow: "הצטרף עכשיו",
    loginAsAdmin: "גישת מנהל",
    demoAccess: "גישת הדגמה",
    chooseYourRole: "בחר את הגישה שלך",
    
    // Features Hebrew
    instantPayouts: "תשלומים מיידיים",
    multiLanguage: "רב לשוני",
    
    // Stats Hebrew
    totalPlayers: "סך השחקנים",
    weeklyJackpot: "ג'קפוט שבועי",
    winnersToDate: "זוכים עד כה",
    
    // Auth Forms Hebrew
    enterCredentials: "הזן את פרטי הגישה שלך",
    emailAddress: "כתובת אימייל",
    passwordField: "סיסמה",
    loginButton: "התחבר",
    registerButton: "הירשם",
    alreadyHaveAccount: "יש לך כבר חשבון?",
    dontHaveAccount: "אין לך חשבון?",
    
    // Admin Interface Hebrew
    adminDashboard: "לוח בקרה מנהל",
    managePlatform: "ניהול פעולות הפלטפורמה",
    totalTickets: "סך הכרטיסים",
    totalRevenue: "סך ההכנסות",
    activeUsers: "משתמשים פעילים",
    completedDraws: "הגרלות שהושלמו",
    
    // Demo Messages Hebrew
    demoMode: "מצב הדגמה",
    demoModeDescription: "זוהי הדגמה עם נתונים לדוגמה",
    
    // Error Messages Hebrew
    loginFailed: "ההתחברות נכשלה. אנא נסה שוב.",
    networkError: "שגיאת רשת. אנא בדוק את החיבור שלך.",
    unexpectedError: "אירעה שגיאה בלתי צפויה.",
    
    // Success Messages Hebrew
    loginSuccess: "התחברות מוצלחת",
    accountCreated: "החשבון נוצר בהצלחה",
    passwordChanged: "הסיסמה שונתה בהצלחה",
    
    // Navigation Hebrew
    backToLogin: "חזרה להתחברות",
    goToHome: "עבור לבית",
    viewProfile: "צפה בפרופיל",
    accountSettings: "הגדרות חשבון",
    
    // Admin Dashboard Missing Keys Hebrew
    jackpot: "ג'קפוט",
    prizePool: "קופת פרסים",
    revenue: "הכנסות",
    ticketSales: "מכירת כרטיסים",
    totalWinners: "סך הזוכים",
    submitDrawResults: "שלח תוצאות הגרלה",
    date: "תאריך",
    selectWinningNumbers: "בחר מספרים זוכים",
    winningNumbers: "מספרים זוכים",
    processing: "מעבד",
    submitResults: "שלח תוצאות",
    drawStatistics: "סטטיסטיקות הגרלה",
    ticketsText: "כרטיסים",
    prizeDistribution: "חלוקת פרסים",
    platformRetention: "החזקת פלטפורמה",
    user: "משתמש",
    drawCreatedSuccessfully: "הגרלה נוצרה בהצלחה",
    selectDrawDate: "בחר תאריך הגרלה",
    createNewDraw: "צור הגרלה חדשה",
    creating: "יוצר",
    createDraw: "צור הגרלה",
    platformRevenue: "הכנסות פלטפורמה",
    retentionFromDraw: "החזקה מהגרלה",
    depositSuccessful: "הפקדה מוצלחת",
    invalidAmount: "סכום לא תקין",
    searchUserByEmail: "חפש משתמש לפי אימייל",
    userNotFound: "משתמש לא נמצא",
    depositAmount: "סכום הפקדה",
    enterAmount: "הזן סכום",
    enterComment: "הזן הערה (אופציונלי)",
    depositing: "מפקיד",
    yourWinningNumbers: "המספרים הזוכים שלך",
    noCompletedDraws: "לא נמצאו הגרלות שהושלמו",
    
    // ===== HEBREW TRANSLATIONS - CLEAN VERSION =====
    
    // ROLE-BASED ACCESS (גישות לפי תפקיד)
    rootAdminPanel: "פאנל מנהל עליון",
    rootAdminAccess: "גישת מנהל עליון",
    systemManagement: "ניהול מערכת",
    globalSettings: "הגדרות גלובליות",
    userRoleManagement: "ניהול תפקידי משתמשים",
    systemSecurity: "אבטחת מערכת",
    databaseManagement: "ניהול בסיס נתונים",
    backupAndRestore: "גיבוי ושחזור",
    systemLogs: "לוגי מערכת",
    globalStatistics: "סטטיסטיקות גלובליות",
    platformConfiguration: "תצורת פלטפורמה",
    rootAdminDashboard: "לוח בקרה מנהל עליון",
    manageAdmins: "ניהול מנהלים",
    createAdmin: "יצירת מנהל",
    deleteAdmin: "מחיקת מנהל",
    adminPermissions: "הרשאות מנהל",
    systemHealth: "בריאות מערכת",
    serverStatus: "סטטוס שרת",
    maintenanceMode: "מצב תחזוקה",
    criticalSettings: "הגדרות קריטיות",
    
    // RÔLE ADMIN STANDARD (מנהל רגיל)
    standardAdminPanel: "פאנל מנהל רגיל",
    standardAdminAccess: "גישת מנהל רגיל",
    clientManagement: "ניהול לקוחות",
    drawManagement: "ניהול הגרלות",
    transactionMonitoring: "מעקב עסקאות",
    reportGeneration: "יצירת דוחות",
    supportManagement: "ניהול תמיכה",
    manageClients: "ניהול לקוחות",
    createClient: "יצירת לקוח",
    blockClient: "חסימת לקוח",
    unblockClient: "ביטול חסימת לקוח",
    viewClientHistory: "צפייה בהיסטוריית לקוח",
    clientTransactions: "עסקאות לקוח",
    manualRefund: "החזר ידני",
    clientSupport: "תמיכת לקוחות",
    messagingSystem: "מערכת הודעות",
    bulkOperations: "פעולות במסה",
    
    // RÔLE CLIENT VIP (לקוח VIP)
    vipClientPanel: "פאנל לקוח VIP",
    vipClientAccess: "גישת לקוח VIP",
    vipBenefits: "הטבות VIP",
    prioritySupport: "תמיכה מועדפת",
    exclusiveDraws: "הגרלות בלעדיות",
    higherLimits: "מגבלות גבוהות יותר",
    personalAccount: "מנהל חשבון אישי",
    vipRewards: "פרסי VIP",
    exclusiveOffers: "הצעות בלעדיות",
    fastWithdrawals: "משיכות מהירות",
    vipChat: "צ'אט VIP",
    dedicatedSupport: "תמיכה ייעודית",
    vipHistory: "היסטוריית VIP",
    bonusProgram: "תוכנית בונוסים",
    loyaltyPoints: "נקודות נאמנות",
    specialEvents: "אירועים מיוחדים",
    
    // RÔLE CLIENT STANDARD (לקוח רגיל)
    standardClientPanel: "פאנל לקוח רגיל",
    standardClientAccess: "גישת לקוח רגיל",
    basicFeatures: "תכונות בסיסיות",
    standardSupport: "תמיכה רגילה",
    regularDraws: "הגרלות רגילות",
    standardLimits: "מגבלות רגילות",
    myAccount: "החשבון שלי",
    playHistory: "היסטוריית משחק",
    winningsHistory: "היסטוריית זכיות",
    accountBalance: "יתרת חשבון",
    depositFunds: "הפקדת כספים",
    withdrawFunds: "משיכת כספים",
    myTickets: "הכרטיסים שלי",
    upcomingDraws: "הגרלות קרובות",
    pastResults: "תוצאות עבר",
    
    // RÔLE NOUVEAU CLIENT (לקוח חדש)
    newClientPanel: "פאנל לקוח חדש",
    newClientAccess: "גישת לקוח חדש",
    welcomeBonus: "בונוס ברוכים הבאים",
    tutorial: "מדריך",
    gettingStarted: "תחילת העבודה",
    firstDeposit: "הפקדה ראשונה",
    learningMode: "מצב למידה",
    helpGuide: "מדריך עזרה",
    basicInstructions: "הוראות בסיסיות",
    accountSetup: "הגדרת חשבון",
    profileCompletion: "השלמת פרופיל",
    verificationProcess: "תהליך אימות",
    welcomeOffer: "הצעת ברוכים הבאים",
    trialPeriod: "תקופת ניסיון",
    introductoryLimits: "מגבלות היכרות",
    
    // Actions et boutons (suppression des doublons)
    
    // États et statuts
    enabled: "מופעל",
    disabled: "מנוטרל",
    available: "זמין",
    unavailable: "לא זמין",
    online: "מחובר",
    offline: "מנותק",
    pending: "ממתין",
    completed: "הושלם",
    failed: "נכשל",
    cancelled: "בוטל",
    expired: "פג תוקף",
    
    // Navigation complémentaire
    menu: "תפריט",
    settings: "הגדרות",
    profile: "פרופיל",
    account: "חשבון",
    preferences: "העדפות",
    notifications: "התראות",
    security: "אבטחה",
    privacy: "פרטיות",
    help: "עזרה",
    about: "אודות",
    terms: "תנאי שימוש",
    policy: "מדיניות פרטיות",
    
    // ===== FONCTIONNALITÉS AVANCÉES =====
    
    // CRYPTO PAYMENTS (תשלומי קריפטו)
    cryptoPayments: "תשלומי קריפטו",
    cryptoDeposit: "הפקדת קריפטו",
    cryptoWithdrawal: "משיכת קריפטו",
    bitcoinPayment: "תשלום ביטקוין",
    ethereumPayment: "תשלום אתריום",
    cryptoWallet: "ארנק קריפטו",
    walletAddress: "כתובת ארנק",
    transactionHash: "האש עסקה",
    cryptoHistory: "היסטוריית קריפטו",
    pendingCrypto: "קריפטו ממתין",
    confirmedCrypto: "קריפטו מאושר",
    cryptoFees: "עמלות קריפטו",
    exchangeRate: "שער חליפין",
    cryptoSupport: "תמיכת קריפטו",
    walletManagement: "ניהול ארנקים",
    cryptoSettings: "הגדרות קריפטו",
    
    // SECURITY FEATURES (תכונות אבטחה)
    securitySettings: "הגדרות אבטחה",
    twoFactorAuth: "אימות דו-שלבי",
    loginHistory: "היסטוריית התחברות",
    securityEvents: "אירועי אבטחה",
    passwordChange: "שינוי סיסמה",
    accountSecurity: "אבטחת חשבון",
    securitySummary: "סיכום אבטחה",
    suspiciousActivity: "פעילות חשודה",
    ipBlacklist: "רשימת IP חסומים",
    sessionManagement: "ניהול הפעלות",
    securityLevel: "רמת אבטחה",
    securityRecommendations: "המלצות אבטחה",
    enableTwoFA: "הפעלת 2FA",
    disableTwoFA: "ביטול 2FA",
    backupCodes: "קודי גיבוי",
    securityScan: "סריקת אבטחה",
    threatDetection: "זיהוי איומים",
    
    // ANALYTICS & REPORTS (אנליטיקה ודוחות)
    analytics: "אנליטיקה",
    reports: "דוחות",
    userAnalytics: "אנליטיקת משתמשים",
    revenueAnalytics: "אנליטיקת הכנסות",
    drawAnalytics: "אנליטיקת הגרלות",
    conversionAnalytics: "אנליטיקת המרות",
    performanceMetrics: "מדדי ביצועים",
    userBehavior: "התנהגות משתמשים",
    trafficAnalysis: "ניתוח תנועה",
    salesReport: "דוח מכירות",
    profitReport: "דוח רווחים",
    userGrowth: "צמיחת משתמשים",
    retentionRate: "שיעור שימור",
    churnAnalysis: "ניתוח נשירה",
    segmentAnalysis: "ניתוח פלחים",
    cohortAnalysis: "ניתוח קבוצות",
    
    // EMAIL SYSTEM (מערכת אימייל)
    emailSystem: "מערכת אימייל",
    emailTemplates: "תבניות אימייל",
    emailConfig: "הגדרות אימייל",
    sendEmail: "שליחת אימייל",
    emailHistory: "היסטוריית אימיילים",
    emailQueue: "תור אימיילים",
    bounceRate: "שיעור החזרות",
    openRate: "שיעור פתיחה",
    clickRate: "שיעור לחיצות",
    unsubscribeRate: "שיעור ביטול הרשמה",
    emailDelivery: "משלוח אימייל",
    smtpSettings: "הגדרות SMTP",
    emailValidation: "אימות אימייל",
    bulkEmail: "אימייל המוני",
    personalizedEmail: "אימייל מותאם אישית",
    autoResponder: "מחזיר אוטומטי",
    
    // SMS SYSTEM (מערכת SMS)
    smsSystem: "מערכת SMS",
    smsNotifications: "התראות SMS",
    smsConfig: "הגדרות SMS",
    sendSMS: "שליחת SMS",
    smsHistory: "היסטוריית SMS",
    smsQueue: "תור SMS",
    smsDelivery: "משלוח SMS",
    smsCredits: "קרדיטים SMS",
    twilioSettings: "הגדרות Twilio",
    smsTemplates: "תבניות SMS",
    bulkSMS: "SMS המוני",
    smsVerification: "אימות SMS",
    phoneVerification: "אימות טלפון",
    smsOTP: "SMS OTP",
    
    // ===== MENUS ET NAVIGATION PAR RÔLE =====
    
    // MENU ROOT ADMIN (תפריט מנהל עליון)
    systemOverview: "סקירת מערכת",
    userRoles: "תפקידי משתמשים",
    permissions: "הרשאות",
    auditLogs: "לוגי ביקורת",
    systemConfig: "תצורת מערכת",
    integrations: "אינטגרציות",
    apiManagement: "ניהול API",
    webhooks: "Webhooks",
    scheduledTasks: "משימות מתוזמנות",
    monitoringDashboard: "לוח מעקב",
    alertsManagement: "ניהול התראות",
    performanceTuning: "כוונון ביצועים",
    
    // MENU ADMIN STANDARD (תפריט מנהל רגיל)
    clientOverview: "סקירת לקוחות",
    ticketManagement: "ניהול כרטיסים",
    drawSchedule: "לוח הגרלות",
    paymentProcessing: "עיבוד תשלומים",
    customerSupport: "שירות לקוחות",
    reportingCenter: "מרכז דיווח",
    contentManagement: "ניהול תוכן",
    promotionsManagement: "ניהול מבצעים",
    notificationCenter: "מרכז התראות",
    moderationTools: "כלי מודרציה",
    
    // MENU CLIENT VIP (תפריט לקוח VIP)
    vipDashboard: "לוח VIP",
    exclusiveGames: "משחקים בלעדיים",
    vipOffers: "הצעות VIP",
    priorityWithdrawals: "משיכות מועדפות",
    personalManager: "מנהל אישי",
    vipEvents: "אירועי VIP",
    premiumSupport: "תמיכה פרימיום",
    advancedStats: "סטטיסטיקות מתקדמות",
    customLimits: "מגבלות מותאמות",
    
    // MENU CLIENT STANDARD (תפריט לקוח רגיל)
    myDashboard: "הלוח שלי",
    playGames: "משחקים",
    myTransactions: "העסקאות שלי",
    accountSettingsMenu: "הגדרות חשבון",
    helpCenterMenu: "מרכז עזרה",
    contactSupport: "צור קשר עם תמיכה",
    promotions: "מבצעים",
    gameHistory: "היסטוריית משחקים",
    
    // MENU NOUVEAU CLIENT (תפריט לקוח חדש)
    welcomeDashboard: "לוח ברוכים הבאים",
    quickStart: "התחלה מהירה",
    learningCenter: "מרכז למידה",
    firstSteps: "הצעדים הראשונים",
    basicSettings: "הגדרות בסיסיות",
    verifyAccount: "אמת חשבון",
    claimBonus: "קבל בונוס",
    practiceMode: "מצב אימון",
    
    // ===== WORKFLOWS ET PROCESSUS =====
    
    // PROCESSUS D'INSCRIPTION (תהליך הרשמה)
    registrationProcess: "תהליך הרשמה",
    personalInfo: "מידע אישי",
    contactInfo: "פרטי קשר",
    accountVerification: "אימות חשבון",
    termsAcceptance: "קבלת תנאים",
    welcomeEmail: "אימייל ברוכים הבאים",
    accountActivation: "הפעלת חשבון",
    
    // PROCESSUS DE VÉRIFICATION (תהליך אימות)
    verificationWorkflow: "תהליך אימות",
    identityVerification: "אימות זהות",
    documentUpload: "העלאת מסמכים",
    manualReview: "בדיקה ידנית",
    autoVerification: "אימות אוטומטי",
    verificationStatus: "סטטוס אימות",
    rejectionReason: "סיבת דחייה",
    resubmission: "הגשה מחדש",
    
    // PROCESSUS DE PAIEMENT (תהליך תשלום)
    paymentProcess: "תהליך תשלום",
    paymentMethods: "שיטות תשלום",
    cardPayment: "תשלום בכרטיס",
    bankTransfer: "העברה בנקאית",
    digitalWallet: "ארנק דיגיטלי",
    paymentSecurity: "אבטחת תשלום",
    fraudPrevention: "מניעת הונאה",
    chargebackProtection: "הגנת חזרה",
    
    // PROCESSUS DE RETRAIT (תהליך משיכה)
    withdrawalProcess: "תהליך משיכה",
    withdrawalMethods: "שיטות משיכה",
    minimumWithdrawal: "משיכה מינימלית",
    maximumWithdrawal: "משיכה מקסימלית",
    withdrawalFees: "עמלות משיכה",
    processingTime: "זמן עיבוד",
    withdrawalHistory: "היסטוריית משיכות",
    pendingWithdrawals: "משיכות ממתינות",
    
    // ===== STATUTS ET ÉTATS =====
    
    // STATUTS UTILISATEUR (סטטוסי משתמש)
    userStatuses: "סטטוסי משתמש",
    activeStatus: "סטטוס פעיל",
    inactiveStatus: "סטטוס לא פעיל",
    suspendedStatus: "סטטוס מושעה",
    bannedStatus: "סטטוס חסום",
    verifiedStatus: "סטטוס מאומת",
    unverifiedStatus: "סטטוס לא מאומת",
    vipStatus: "סטטוס VIP",
    premiumStatus: "סטטוס פרימיום",
    
    // ÉTATS DES TRANSACTIONS (מצבי עסקאות)
    transactionStates: "מצבי עסקאות",
    pendingTransaction: "עסקה ממתינה",
    completedTransaction: "עסקה הושלמה",
    failedTransaction: "עסקה נכשלה",
    cancelledTransaction: "עסקה בוטלה",
    processingTransaction: "עסקה מעובדת",
    refundedTransaction: "עסקה הוחזרה",
    
    // ÉTATS DES TIRAGES (מצבי הגרלות)
    drawStates: "מצבי הגרלות",
    upcomingDraw: "הגרלה קרובה",
    activeDraw: "הגרלה פעילה",
    lockedDraw: "הגרלה נעולה",
    completedDraw: "הגרלה הושלמה",
    cancelledDraw: "הגרלה בוטלה",
    
    // ===== NOTIFICATIONS ET ALERTES =====
    
    // TYPES DE NOTIFICATIONS (סוגי התראות)
    notificationTypes: "סוגי התראות",
    systemNotifications: "התראות מערכת",
    paymentNotifications: "התראות תשלום",
    gameNotifications: "התראות משחק",
    securityNotifications: "התראות אבטחה",
    promotionalNotifications: "התראות פרסומיות",
    urgentNotifications: "התראות דחופות",
    infoNotifications: "התראות מידע",
    warningNotifications: "התראות אזהרה",
    
    // CANAUX DE NOTIFICATION (ערוצי התראה)
    notificationChannels: "ערוצי התראה",
    emailNotifications: "התראות אימייל",
    smsAlerts: "התראות SMS",
    pushNotifications: "התראות דחיפה",
    inAppNotifications: "התראות באפליקציה",
    browserNotifications: "התראות דפדפן",
    whatsappNotifications: "התראות ווטסאפ",
    telegramNotifications: "התראות טלגרם",
    
    // ===== PARAMÈTRES ET CONFIGURATION =====
    
    // PARAMÈTRES GÉNÉRAUX (הגדרות כלליות)
    generalSettings: "הגדרות כלליות",
    languageSettings: "הגדרות שפה",
    timezoneSettings: "הגדרות איזור זמן",
    currencySettings: "הגדרות מטבע",
    displaySettings: "הגדרות תצוגה",
    themeSettings: "הגדרות נושא",
    accessibilitySettings: "הגדרות נגישות",
    privacySettings: "הגדרות פרטיות",
    
    // PARAMÈTRES DE SÉCURITÉ (הגדרות אבטחה)
    securityConfiguration: "תצורת אבטחה",
    passwordPolicy: "מדיניות סיסמאות",
    sessionTimeout: "פסק זמן הפעלה",
    ipRestrictions: "הגבלות IP",
    deviceManagement: "ניהול מכשירים",
    loginAttempts: "ניסיונות התחברות",
    accountLockout: "נעילת חשבון",
    securityQuestions: "שאלות אבטחה",
    
    // PARAMÈTRES DE JEU (הגדרות משחק)
    gameSettings: "הגדרות משחק",
    betLimits: "מגבלות הימור",
    playTime: "זמן משחק",
    responsibleGaming: "משחק אחראי",
    selfExclusion: "הרחקה עצמית",
    cooloffPeriod: "תקופת הפוגה",
    realityCheck: "בדיקת מציאות",
    spendingLimits: "מגבלות הוצאה",
    
    // ===== AIDE ET SUPPORT =====
    
    // CENTRE D'AIDE (מרכז עזרה)
    helpCenterMain: "מרכז עזרה",
    faqSection: "מדור שאלות נפוצות",
    tutorials: "מדריכים",
    videoGuides: "מדריכי וידאו",
    stepByStepGuides: "מדריכים שלב אחר שלב",
    troubleshooting: "פתרון בעיות",
    knowledgeBase: "בסיס ידע",
    searchHelp: "חיפוש בעזרה",
    
    // SUPPORT CLIENT (תמיכת לקוחות)
    customerSupportMain: "תמיכת לקוחות",
    liveChatSupport: "תמיכת צ'אט חי",
    emailSupport: "תמיכת אימייל",
    phoneSupport: "תמיכה טלפונית",
    ticketSupport: "תמיכת כרטיסים",
    prioritySupportService: "תמיכה מועדפת",
    supportHours: "שעות תמיכה",
    supportLanguages: "שפות תמיכה",
    
    // TYPES DE DEMANDES (סוגי בקשות)
    requestTypes: "סוגי בקשות",
    technicalSupport: "תמיכה טכנית",
    accountIssues: "בעיות חשבון",
    paymentIssues: "בעיות תשלום",
    gameIssues: "בעיות משחק",
    generalInquiry: "פנייה כללית",
    complaintSubmission: "הגשת תלונה",
    suggestionBox: "תיבת הצעות",
    feedbackForm: "טופס משוב",
    
    // LANDING PAGE MISSING TRANSLATIONS - HEBREW
    massiveJackpots: "ג'קפוטים ענקיים",
    lifeChangingPrizes: "פרסים שמשנים חיים בכל הגרלה",
    advancedSecurityPayouts: "אבטחה מתקדמת ותשלומים מיידיים",
    referralRewards: "פרסי הפניות",
    earnForReferrals: "הרוויח ₪100 עבור כל חבר שאתה מפנה",
    multipleChancesToWin: "מספר הזדמנויות לזכות כל שבוע",
    readyToChangeLife: "מוכן לשנות את החיים שלך?",
    joinTodayAndStart: "הצטרף היום והתחל את המסע שלך לעושר",
    startPlayingNow: "התחל לשחק עכשיו",
    
    // MISSING HEBREW TRANSLATIONS
    accessDenied: "גישה נדחתה",
    adminAccess: "גישת מנהל",
    adminCredentials: "פרטי גישה מנהל",
    adminLogin: "התחברות מנהל",
    adminLoginDescription: "הזן את פרטי הגישה שלך כדי לגשת לפאנל הניהול",
    adminLoginWelcome: "ברוכים הבאים לפורטל המנהלים",
    adminSignIn: "התחברות מנהל",
    authenticationFailed: "אימות נכשל",
    clientLoginWelcome: "ברוכים הבאים לפורטל הלקוחות",
    emailRequired: "אימייל נדרש",
    enterEmail: "הזן אימייל",
    enterPassword: "הזן סיסמה",
    finalCleanTranslations: "תרגומים סופיים נקיים",
    invalidCredentials: "פרטי גישה לא תקינים",
    loginError: "שגיאת התחברות",
    passwordRequired: "סיסמה נדרשת",
    registrationSuccessful: "הרשמה מוצלחת",
    secureLogin: "התחברות מאובטחת",
    verifyCredentials: "אמת פרטי גישה",
    backupCodes: "קודי גיבוי",
    basicSettings: "הגדרות בסיסיות",
    clientOverview: "סקירת לקוח",
    exclusiveGames: "משחקים בלעדיים",
    failedTransaction: "עסקה נכשלה",
    finalHebrew: "עברית מושלמת",
    firstSteps: "צעדים ראשונים",
    fraudReport: "דוח הונאה",
    gameRules: "חוקי המשחק",
    highRisk: "סיכון גבוה",
    insufficientFunds: "אין מספיק כספים",
    inviteBonus: "בונוס הזמנה",
    lastSeen: "נראה לאחרונה",
    litecoinPayment: "תשלום לייטקוין",
    lowRisk: "סיכון נמוך",
    memberSince: "חבר מאז",
    moderateRisk: "סיכון בינוני",
    newUser: "משתמש חדש",
    onHold: "בהמתנה",
    overdue: "באיחור",
    paymentConfirmation: "אישור תשלום",
    paymentHistory: "היסטוריית תשלומים",
    phoneNumber: "מספר טלפון",
    premiumFeatures: "תכונות פרימיום",
    processingError: "שגיאת עיבוד",
    quickActions: "פעולות מהירות",
    raceToWin: "מירוץ לזכייה",
    refunded: "הוחזר",
    resendCode: "שלח קוד שוב",
    retryPayment: "נסה תשלום שוב",
    securityEvent: "אירוע אבטחה",
    sendMessage: "שלח הודעה",
    settingsPanel: "פאנל הגדרות",
    smartContract: "חוזה חכם",
    socialMedia: "מדיה חברתית",
    successMessage: "הודעת הצלחה",
    supportTeam: "צוות תמיכה",
    taxInfo: "מידע מס",
    temporaryBan: "חסימה זמנית",
    unconfirmed: "לא מאושר",
    upcomingFeatures: "תכונות עתידיות",
    userActivity: "פעילות משתמש",
    verification: "אימות",
    viewDetails: "צפה בפרטים",
    weeklyReport: "דוח שבועי",
    withdrawalError: "שגיאת משיכה",
    yearToDate: "מתחילת השנה",
    
    // FINAL HEBREW MISSING TRANSLATIONS
    errorLogs: "לוגי שגיאות",
    eventTracking: "מעקב אירועים",
    excludedUser: "משתמש מוחרג",
    expiredDraw: "הגרלה שפגה",
    exportData: "יצוא נתונים",
    fraudDetection: "זיהוי הונאות",
    gameConfiguration: "תצורת משחק",
    geoBlocking: "חסימה גיאוגרפית",
    historyTracking: "מעקב היסטוריה",
    incompleteTransaction: "עסקה לא הושלמה",
    lotteryOperations: "פעולות לוטו",
    maintenanceNotification: "הודעת תחזוקה",
    marketingCampaigns: "קמפיינים שיווקיים",
    multipleDevices: "מכשירים מרובים",
    networkIssues: "בעיות רשת",
    operationalLogs: "לוגים תפעוליים",
    personalizedOffers: "הצעות מותאמות אישית",
    riskAssessment: "הערכת סיכונים",
    suspendedUser: "משתמש מושעה",
    cardPayment: "תשלום בכרטיס",
    bankTransfer: "העברה בנקאית",
    
    // ===== FINAL MISSING HEBREW TRANSLATIONS FOR 100% COVERAGE =====
    dashboardActions: "פעולות לוח בקרה",
    jackpotNotifications: "התראות ג'קפוט",
    kycVerification: "אימות זהות",
    loginToAccount: "התחברות לחשבון",
    loyaltyProgram: "תוכנית נאמנות",
    mobileApp: "אפליקציה לנייד",
    multiCurrency: "מטבעות מרובים",
    noAccount: "אין חשבון",
    onlineStatus: "סטטוס מקוון",
    or: "או",
    learningCenter: "מרכז למידה",
    lockedDraw: "הגרלה נעולה",
    loginHistory: "היסטוריית התחברויות",
    manualReview: "בדיקה ידנית",
    maximumWithdrawal: "משיכה מקסימלית",
    minimumWithdrawal: "משיכה מינימלית",
    moderationTools: "כלי מיתון",
    monitoringDashboard: "לוח בקרה ניטור",
    myDashboard: "הלוח שלי",
    myTransactions: "העסקאות שלי",
    notificationCenter: "מרכז התראות",
    onboardingGuide: "מדריך הטמעה",
    operationalDashboard: "לוח בקרה תפעולי",
    paymentProcessor: "מעבד תשלומים",
    permissionControl: "בקרת הרשאות",
    playerStatistics: "סטטיסטיקות שחקנים",
    qualityAssurance: "בטיחות איכות",
    reconciliationReport: "דוח התאמות",
    recurringPayment: "תשלום חוזר",
    referralTracking: "מעקב הפניות",
    reportDownload: "הורדת דוח",
    revenueStream: "זרם הכנסות",
    riskManagement: "ניהול סיכונים",
    schedulePayment: "תזמון תשלום",
    securityProtocol: "פרוטוקול אבטחה",
    serviceStatus: "מצב שירות",
    stakeholderReport: "דוח מחזיקי עניין",
    systemAlert: "התראת מערכת",
    timelineView: "תצוגת ציר זמן",
    transactionFee: "עמלת עסקה",
    userEngagement: "מעורבות משתמשים",
    validationRule: "כלל אימות",
    workflowAutomation: "אוטומציה זרימת עבודה",
    
    // Additional Hebrew Missing Keys for 100% Coverage
    paymentGateway: "שער תשלומים",
    pendingDraw: "הגרלה ממתינה",
    playerRanking: "דירוג שחקנים",
    quickSupport: "תמיכה מהירה",
    realTimeUpdates: "עדכונים בזמן אמת",
    registerHere: "הירשם כאן",
    reportIssue: "דווח על בעיה",
    secureAndTrusted: "בטוח ומהימן",
    securityAlerts: "התראות אבטחה",
    signIn: "כניסה",
    smartphoneApp: "אפליקציית סמארטפון",
    systemMaintenance: "תחזוקת מערכת",
    thirdPartyIntegrations: "שילובי צד שלישי",
    userFeedback: "משוב משתמשים",
    userPreferences: "העדפות משתמש",
    userSettings: "הגדרות משתמש",
    versionUpdate: "עדכון גרסה",
    webNotifications: "התראות אינטרנט",
    welcomePackage: "חבילת ברוכים הבאים",
    winNotifications: "התראות זכייה",
    massiveJackpots: "ג'קפוטים עצומים",
    lifeChangingPrizes: "פרסים משני חיים",
    advancedSecurityPayouts: "אבטחה מתקדמת ותשלומים מיידיים",
    startPlayingNow: "התחל לשחק עכשיו",
    weeklyDraws: "הגרלות שבועיות",
    
    // ===== FINAL REMAINING HEBREW TRANSLATIONS FOR 100% COVERAGE =====
    signingIn: "נכנס למערכת",
    transactionHistory: "היסטוריית עסקאות",
    transactionLimits: "מגבלות עסקאות",
    tryDemo: "נסה דמו",
    twoFactorAuthentication: "אימות דו-שלבי",
    verificationRequired: "נדרש אימות",
    withdrawalLimits: "מגבלות משיכה"
  },
  
  fr: {
    // Application Name
    appName: "Bracha veHatzlacha",
    appNameShort: "Bracha",
    
    // Navigation & General
    home: "Accueil",
    dashboard: "Espace Personnel",
    chat: "Chat et Support",
    admin: "Panneau Admin",
    logout: "Déconnexion",
    login: "Connexion",
    loginAsClient: "Entrer comme Client",
    
    // Home Screen
    welcomeMessage: "Bienvenue au Loto Bracha veHatzlacha",
    participateInLottery: "Participer au Loto",
    selectNumbers: "Sélectionner Numéros",
    selectAmount: "Sélectionner Montant",
    nextDraw: "Prochain Tirage",
    timeUntilDraw: "Temps jusqu'au Tirage",
    jackpotAmount: "Montant Jackpot",
    quickPick: "Sélection Rapide",
    mustSelect6Numbers: "Vous devez sélectionner 6 numéros",
    currentJackpot: "Jackpot Actuel",
    drawNumber: "Tirage #",
    participationHistory: "Historique Participation",
    yourNumbers: "Vos Numéros",
    result: "Résultat",
    winnings: "Gains",
    
    // Personal Area
    currentBalance: "Solde Actuel",
    topupHistory: "Historique Recharges",
    userStatus: "Statut Utilisateur",
    statusNew: "Nouvel Utilisateur",
    statusActive: "Actif",
    statusSilver: "Argent",
    statusGold: "Or",
    statusDiamond: "Diamant",
    statusVIP: "VIP",
    silverRequirement: "10 participations pour statut Argent",
    goldRequirement: "100 participations pour statut Or",
    diamondRequirement: "500 participations pour statut Diamant",
    
    // Chat & Support
    liveChat: "Chat en Direct",
    supportRequest: "Ouvrir Demande Support",
    chatWithSupport: "Chat avec Support",
    typeMessage: "Tapez votre message...",
    send: "Envoyer",
    newSupportRequest: "Nouvelle Demande Support",
    describeProblem: "Décrivez votre problème",
    
    // Notifications
    drawImminent: "Le tirage commence bientôt!",
    winningNotification: "Félicitations! Vous avez gagné ₪{amount}",
    claimInstructions: "Contactez le support pour réclamer votre prix",
    participationConfirmed: "Participation confirmée",
    insufficientBalance: "Solde insuffisant",
    
    // Referral System
    referralProgram: "Programme de Parrainage",
    yourReferralLink: "Votre Lien de Parrainage",
    generateQRCode: "Générer Code QR",
    referralBonus: "Bonus Parrainage: ₪100",
    bonusCondition: "Quand l'ami recharge ₪1000+",
    copyLink: "Copier Lien",
    shareLink: "Partager Lien",
    
    // Winners Display
    recentWinners: "Gagnants Récents",
    winnersCarousel: "Gagnants Récents",
    congratulations: "Félicitations",
    winner: "Gagnant",
    amount: "Montant",
    
    // Quick Contact
    contactWhatsApp: "WhatsApp",
    contactTelegram: "Telegram",
    contactChat: "Chat Direct",
    
    // Admin Panel
    adminPanel: "Panneau Admin",
    platformManagement: "Gestion Plateforme Bracha veHatzlacha",
    userManagement: "Gestion Utilisateurs",
    createUser: "Créer Utilisateur",
    manualDeposit: "Dépôt Manuel",
    systemControls: "Contrôles Système",
    currentDraw: "Tirage Actuel",
    runDraw: "Lancer Tirage",
    drawCompleted: "Tirage Terminé",
    totalUsers: "Total Utilisateurs",
    totalDraws: "Total Tirages",
    usersList: "Liste Utilisateurs",
    name: "Nom",
    email: "Email",
    balance: "Solde",
    status: "Statut",
    actions: "Actions",
    active: "Actif",
    blocked: "Bloqué",
    block: "Bloquer",
    unblock: "Débloquer",
    loading: "Chargement...",
    
    // Admin Messages
    userCreated: "Utilisateur créé avec succès",
    depositSuccess: "Dépôt effectué avec succès",
    userBlocked: "Utilisateur bloqué avec succès",
    userUnblocked: "Utilisateur débloqué avec succès",
    errorLoading: "Erreur de chargement",
    errorCreating: "Erreur création utilisateur",
    errorDeposit: "Erreur traitement dépôt",
    errorBlocking: "Erreur blocage utilisateur",
    errorDraw: "Erreur lancement tirage",
    connectionError: "Erreur de connexion",
    
    // Admin Actions
    deposit: "Dépôt",
    username: "Nom d'utilisateur",
    selectUser: "Sélectionner Utilisateur",
    comment: "Commentaire",
    firstName: "Prénom",
    lastName: "Nom de famille",
    password: "Mot de passe",
    create: "Créer",
    save: "Sauvegarder",
    cancel: "Annuler",
    edit: "Modifier",
    delete: "Supprimer",
    confirm: "Confirmer",
    
    // Validation & Errors
    required: "Champ requis",
    invalidInput: "Saisie invalide",
    error: "Erreur",
    success: "Succès",
    loginRequired: "Connexion requise",
    
    // Time & Dates
    days: "jours",
    hours: "heures",
    minutes: "minutes",
    seconds: "secondes",
    
    // Currency
    currency: "₪",
    shekel: "Shekel",
    
    // Actions
    participate: "Participer",
    topUp: "Recharger",
    claim: "Réclamer Prix",
    contact: "Contact",
    share: "Partager",
    copy: "Copier",
    copied: "Copié!",
    
    // User Status Levels
    userLevel: "Niveau Utilisateur",
    levelProgress: "Progression Niveau",
    nextLevelRequirement: "Exigence Niveau Suivant",
    
    // Previous Numbers History
    previousDrawNumbers: "Numéros Tirages Précédents",
    usePreviousDrawNumbers: "Utiliser Numéros Précédents",
    selectFromDrawHistory: "Sélectionner depuis Historique",
    drawHistoryList: "Liste Historique Tirages",
    pastWinningNumbers: "Numéros Gagnants Passés",
    
    // Contact Widgets
    quickContactWidget: "Contact Rapide",
    whatsappContact: "WhatsApp",
    telegramContact: "Telegram",
    liveChatContact: "Chat Direct",
    
    // Draw Lock System
    drawLockWarning: "Le tirage commence bientôt",
    lockMessage: "Participation verrouillée 60 secondes avant tirage",
    remainingTime: "Temps restant",
    
    // Notification System
    notificationSettings: "Paramètres Notifications",
    drawStartNotification: "Le tirage commence bientôt!",
    winningNotificationText: "Félicitations! Vous avez gagné",
    
    // ===== COMPREHENSIVE FRENCH TRANSLATIONS TO MATCH ENGLISH/HEBREW =====
    
    // Landing Page & Authentication French
    massiveJackpots: "Jackpots Massifs",
    lifeChangingPrizes: "Prix qui changent la vie à chaque tirage",
    advancedSecurityPayouts: "Sécurité avancée et paiements instantanés",
    referralRewards: "Récompenses de Parrainage",
    earnForReferrals: "Gagnez ₪100 pour chaque ami que vous parrainez",
    multipleChancesToWin: "Plusieurs chances de gagner chaque semaine",
    readyToChangeLife: "Prêt à Changer Votre Vie?",
    joinTodayAndStart: "Rejoignez Bracha veHatzlacha aujourd'hui et commencez votre voyage vers la fortune",
    startPlayingNow: "Commencer à Jouer Maintenant",
    weeklyDraws: "Tirages Hebdomadaires",
    secureAndTrusted: "100% Sécurisé",
    
    // Authentication & Access French
    adminCredentials: "Identifiants Admin",
    adminLogin: "Connexion Admin",
    adminLoginDescription: "Entrez vos identifiants admin pour accéder au panneau de gestion",
    adminLoginWelcome: "Bienvenue au Portail Admin",
    adminAccess: "Accès Admin",
    adminSignIn: "Connexion Admin",
    clientLoginWelcome: "Bienvenue au Portail Client",
    enterEmail: "Entrer Email",
    enterPassword: "Entrer Mot de Passe",
    loginError: "Erreur de Connexion",
    authenticationFailed: "Authentification Échouée",
    secureLogin: "Connexion Sécurisée",
    verifyCredentials: "Vérifier Identifiants",
    accessDenied: "Accès Refusé",
    invalidCredentials: "Identifiants Invalides",
    passwordRequired: "Mot de Passe Requis",
    emailRequired: "Email Requis",
    registrationSuccessful: "Inscription Réussie",
    finalCleanTranslations: "Traductions Finales Propres",
    
    // Advanced Features French
    fraudPrevention: "Prévention de la Fraude",
    gameHistory: "Historique des Jeux",
    gameNotifications: "Notifications de Jeux",
    help: "Aide",
    helpCenterMenu: "Menu Centre d'Aide",
    identityVerification: "Vérification d'Identité",
    inactiveStatus: "Statut Inactif",
    infoNotifications: "Notifications d'Info",
    integrations: "Intégrations",
    ipBlacklist: "Liste Noire IP",
    jackpotNotifications: "Notifications Jackpot",
    kycVerification: "Vérification KYC",
    loyaltyProgram: "Programme de Fidélité",
    menu: "Menu",
    mobileApp: "Application Mobile",
    multiCurrency: "Multi-Devise",
    notifications: "Notifications",
    onlineStatus: "Statut En Ligne",
    paymentGateway: "Passerelle de Paiement",
    pendingDraw: "Tirage en Attente",
    personalInfo: "Info Personnelle",
    playerRanking: "Classement des Joueurs",
    policy: "Politique",
    preferences: "Préférences",
    privacy: "Confidentialité",
    profile: "Profil",
    promotions: "Promotions",
    quickSupport: "Support Rapide",
    realTimeUpdates: "Mises à Jour en Temps Réel",
    reportIssue: "Signaler un Problème",
    securityAlerts: "Alertes de Sécurité",
    smartphoneApp: "Application Smartphone",
    systemMaintenance: "Maintenance Système",
    terms: "Conditions",
    thirdPartyIntegrations: "Intégrations Tierces",
    transactionHistory: "Historique des Transactions",
    userFeedback: "Commentaires Utilisateur",
    userPreferences: "Préférences Utilisateur",
    userSettings: "Paramètres Utilisateur",
    versionUpdate: "Mise à Jour Version",
    webNotifications: "Notifications Web",
    welcomePackage: "Pack de Bienvenue",
    winNotifications: "Notifications de Gains",
    
    // Role-Based Access French
    rootAdminPanel: "Panneau Admin Principal",
    rootAdminAccess: "Accès Admin Principal",
    systemManagement: "Gestion Système",
    globalSettings: "Paramètres Globaux",
    userRoleManagement: "Gestion des Rôles Utilisateur",
    systemSecurity: "Sécurité Système",
    databaseManagement: "Gestion Base de Données",
    backupAndRestore: "Sauvegarde et Restauration",
    systemLogs: "Journaux Système",
    globalStatistics: "Statistiques Globales",
    platformConfiguration: "Configuration Plateforme",
    rootAdminDashboard: "Tableau de Bord Admin Principal",
    manageAdmins: "Gérer Admins",
    createAdmin: "Créer Admin",
    deleteAdmin: "Supprimer Admin",
    adminPermissions: "Permissions Admin",
    systemHealth: "Santé Système",
    serverStatus: "Statut Serveur",
    maintenanceMode: "Mode Maintenance",
    criticalSettings: "Paramètres Critiques",
    
    // Standard Admin French
    standardAdminPanel: "Panneau Admin Standard",
    standardAdminAccess: "Accès Admin Standard",
    clientManagement: "Gestion Clients",
    drawManagement: "Gestion Tirages",
    transactionMonitoring: "Surveillance Transactions",
    reportGeneration: "Génération Rapports",
    supportManagement: "Gestion Support",
    manageClients: "Gérer Clients",
    createClient: "Créer Client",
    blockClient: "Bloquer Client",
    unblockClient: "Débloquer Client",
    viewClientHistory: "Voir Historique Client",
    clientTransactions: "Transactions Client",
    manualRefund: "Remboursement Manuel",
    clientSupport: "Support Client",
    messagingSystem: "Système de Messagerie",
    bulkOperations: "Opérations en Masse",
    
    // VIP Client French
    vipClientPanel: "Panneau Client VIP",
    vipClientAccess: "Accès Client VIP",
    vipBenefits: "Avantages VIP",
    prioritySupport: "Support Prioritaire",
    exclusiveDraws: "Tirages Exclusifs",
    higherLimits: "Limites Plus Élevées",
    personalAccount: "Gestionnaire de Compte Personnel",
    vipRewards: "Récompenses VIP",
    exclusiveOffers: "Offres Exclusives",
    fastWithdrawals: "Retraits Rapides",
    vipChat: "Chat VIP",
    dedicatedSupport: "Support Dédié",
    vipHistory: "Historique VIP",
    bonusProgram: "Programme Bonus",
    loyaltyPoints: "Points de Fidélité",
    specialEvents: "Événements Spéciaux",
    
    // Standard Client French
    standardClientPanel: "Panneau Client Standard",
    standardClientAccess: "Accès Client Standard",
    basicFeatures: "Fonctionnalités de Base",
    standardSupport: "Support Standard",
    regularDraws: "Tirages Réguliers",
    standardLimits: "Limites Standard",
    myAccount: "Mon Compte",
    playHistory: "Historique de Jeu",
    winningsHistory: "Historique des Gains",
    accountBalance: "Solde du Compte",
    depositFunds: "Déposer des Fonds",
    withdrawFunds: "Retirer des Fonds",
    myTickets: "Mes Tickets",
    upcomingDraws: "Tirages à Venir",
    pastResults: "Résultats Passés",
    
    // New Client French
    newClientPanel: "Panneau Nouveau Client",
    newClientAccess: "Accès Nouveau Client",
    welcomeBonus: "Bonus de Bienvenue",
    tutorial: "Tutoriel",
    gettingStarted: "Commencer",
    firstDeposit: "Premier Dépôt",
    learningMode: "Mode Apprentissage",
    helpGuide: "Guide d'Aide",
    basicInstructions: "Instructions de Base",
    accountSetup: "Configuration Compte",
    profileCompletion: "Complétion Profil",
    verificationProcess: "Processus de Vérification",
    welcomeOffer: "Offre de Bienvenue",
    trialPeriod: "Période d'Essai",
    introductoryLimits: "Limites d'Introduction",
    
    // States and Statuses French
    enabled: "Activé",
    disabled: "Désactivé",
    available: "Disponible",
    unavailable: "Indisponible",
    online: "En Ligne",
    offline: "Hors Ligne",
    pending: "En Attente",
    completed: "Terminé",
    failed: "Échoué",
    cancelled: "Annulé",
    expired: "Expiré",
    
    // Additional System Features French
    about: "À Propos",
    account: "Compte",
    accountActivation: "Activation Compte",
    accountIssues: "Problèmes de Compte",
    accountLockout: "Verrouillage Compte",
    accountSecurity: "Sécurité Compte",
    accountSettingsMenu: "Paramètres Compte",
    accountVerification: "Vérification Compte",
    activeDraw: "Tirage Actif",
    activeStatus: "Statut Actif",
    advancedStats: "Statistiques Avancées",
    alertsManagement: "Gestion des Alertes",
    analytics: "Analytiques",
    apiManagement: "Gestion API",
    auditLogs: "Journaux d'Audit",
    autoResponder: "Répondeur Automatique",
    autoVerification: "Vérification Automatique",
    bankTransfer: "Virement Bancaire",
    bannedStatus: "Statut Banni",
    betLimits: "Limites de Paris",
    bitcoinPayment: "Paiement Bitcoin",
    bounceRate: "Taux de Rebond",
    browserNotifications: "Notifications Navigateur",
    bulkEmail: "Email en Masse",
    bulkSMS: "SMS en Masse",
    cancelledDraw: "Tirage Annulé",
    cancelledTransaction: "Transaction Annulée",
    cardPayment: "Paiement par Carte",
    chargebackProtection: "Protection Rétrofacturation",
    churnAnalysis: "Analyse d'Attrition",
    claimBonus: "Réclamer Bonus",
    clickRate: "Taux de Clics",
    cohortAnalysis: "Analyse de Cohorte",
    complaintSubmission: "Soumission de Plainte",
    completedDraw: "Tirage Terminé",
    completedTransaction: "Transaction Terminée",
    confirmedCrypto: "Crypto Confirmé",
    contactInfo: "Infos Contact",
    contactSupport: "Contacter Support",
    contentManagement: "Gestion de Contenu",
    conversionAnalytics: "Analytiques de Conversion",
    cooloffPeriod: "Période de Refroidissement",
    cryptoDeposit: "Dépôt Crypto",
    cryptoFees: "Frais Crypto",
    cryptoHistory: "Historique Crypto",
    cryptoPayments: "Paiements Crypto",
    cryptoSettings: "Paramètres Crypto",
    cryptoSupport: "Support Crypto",
    cryptoWallet: "Portefeuille Crypto",
    cryptoWithdrawal: "Retrait Crypto",
    currencySettings: "Paramètres Devise",
    customLimits: "Limites Personnalisées",
    customerSupport: "Support Client",
    customerSupportMain: "Support Client Principal",
    deviceManagement: "Gestion Appareils",
    digitalWallet: "Portefeuille Numérique",
    disableTwoFA: "Désactiver 2FA",
    documentUpload: "Téléchargement Document",
    drawAnalytics: "Analytiques Tirage",
    drawSchedule: "Calendrier Tirages",
    drawStates: "États Tirage",
    emailConfig: "Configuration Email",
    emailDelivery: "Livraison Email",
    emailHistory: "Historique Email",
    emailNotifications: "Notifications Email",
    emailQueue: "Queue Email",
    emailSupport: "Support Email",
    emailSystem: "Système Email",
    emailTemplates: "Modèles Email",
    emailValidation: "Validation Email",
    enableTwoFA: "Activer 2FA",
    errorLogs: "Journaux d'Erreur",
    eventTracking: "Suivi d'Événements",
    excludedUser: "Utilisateur Exclu",
    expiredDraw: "Tirage Expiré",
    exportData: "Exporter Données",
    faqSection: "Section FAQ",
    feedbackForm: "Formulaire Commentaires",
    fraudDetection: "Détection Fraude",
    gameConfiguration: "Configuration Jeu",
    gameIssues: "Problèmes de Jeu",
    gameSettings: "Paramètres Jeu",
    generalInquiry: "Demande Générale",
    generalSettings: "Paramètres Généraux",
    geoBlocking: "Blocage Géographique",
    helpCenterMain: "Centre d'Aide Principal",
    historyTracking: "Suivi Historique",
    inAppNotifications: "Notifications In-App",
    incompleteTransaction: "Transaction Incomplète",
    ipRestrictions: "Restrictions IP",
    knowledgeBase: "Base de Connaissances",
    languageSettings: "Paramètres Langue",
    liveChatSupport: "Support Chat Direct",
    loginAttempts: "Tentatives de Connexion",
    lotteryOperations: "Opérations Loterie",
    maintenanceNotification: "Notification Maintenance",
    marketingCampaigns: "Campagnes Marketing",
    multipleDevices: "Appareils Multiples",
    networkIssues: "Problèmes Réseau",
    notificationChannels: "Canaux Notification",
    operationalLogs: "Journaux Opérationnels",
    passwordPolicy: "Politique Mot de Passe",
    paymentIssues: "Problèmes Paiement",
    paymentMethods: "Méthodes Paiement",
    pendingTransaction: "Transaction en Attente",
    personalizedOffers: "Offres Personnalisées",
    phoneSupport: "Support Téléphonique",
    playTime: "Temps de Jeu",
    prioritySupportService: "Service Support Prioritaire",
    privacySettings: "Paramètres Confidentialité",
    pushNotifications: "Notifications Push",
    realityCheck: "Vérification Réalité",
    responsibleGaming: "Jeu Responsable",
    riskAssessment: "Évaluation Risque",
    searchHelp: "Rechercher Aide",
    securityConfiguration: "Configuration Sécurité",
    securityQuestions: "Questions Sécurité",
    selfExclusion: "Auto-Exclusion",
    sessionTimeout: "Délai Session",
    smsAlerts: "Alertes SMS",
    spendingLimits: "Limites Dépenses",
    stepByStepGuides: "Guides Étape par Étape",
    suggestionBox: "Boîte à Suggestions",
    supportHours: "Heures Support",
    supportLanguages: "Langues Support",
    suspendedUser: "Utilisateur Suspendu",
    telegramNotifications: "Notifications Telegram",
    themeSettings: "Paramètres Thème",
    ticketSupport: "Support Tickets",
    timezoneSettings: "Paramètres Fuseau Horaire",
    transactionLimits: "Limites Transaction",
    troubleshooting: "Dépannage",
    tutorials: "Tutoriels",
    twoFactorAuthentication: "Authentification Deux Facteurs",
    videoGuides: "Guides Vidéo",
    verificationRequired: "Vérification Requise",
    whatsappNotifications: "Notifications WhatsApp",
    withdrawalLimits: "Limites Retrait",
    requestTypes: "Types Demandes",
    technicalSupport: "Support Technique",
    displaySettings: "Paramètres Affichage",
    accessibilitySettings: "Paramètres Accessibilité",
    
    // Final Additional French Translations
    backupCodes: "Codes de Sauvegarde",
    basicSettings: "Paramètres de Base",
    clientOverview: "Aperçu Client",
    ethereumPayment: "Paiement Ethereum",
    exchangeRate: "Taux de Change",
    exclusiveGames: "Jeux Exclusifs",
    failedTransaction: "Transaction Échouée",
    finalHebrew: "Hébreu Parfait",
    firstSteps: "Premiers Pas",
    fraudReport: "Rapport Fraude",
    gameRules: "Règles du Jeu",
    highRisk: "Risque Élevé",
    insufficientFunds: "Fonds Insuffisants",
    inviteBonus: "Bonus Invitation",
    lastSeen: "Vu Dernièrement",
    litecoinPayment: "Paiement Litecoin",
    lowRisk: "Risque Faible",
    memberSince: "Membre Depuis",
    moderateRisk: "Risque Modéré",
    newUser: "Nouvel Utilisateur",
    onHold: "En Attente",
    overdue: "En Retard",
    paymentConfirmation: "Confirmation Paiement",
    paymentHistory: "Historique Paiements",
    phoneNumber: "Numéro Téléphone",
    premiumFeatures: "Fonctionnalités Premium",
    processingError: "Erreur Traitement",
    quickActions: "Actions Rapides",
    raceToWin: "Course pour Gagner",
    refunded: "Remboursé",
    resendCode: "Renvoyer Code",
    retryPayment: "Réessayer Paiement",
    securityEvent: "Événement Sécurité",
    sendMessage: "Envoyer Message",
    settingsPanel: "Panneau Paramètres",
    smartContract: "Contrat Intelligent",
    socialMedia: "Médias Sociaux",
    successMessage: "Message de Succès",
    supportTeam: "Équipe Support",
    taxInfo: "Infos Taxes",
    temporaryBan: "Bannissement Temporaire",
    unconfirmed: "Non Confirmé",
    upcomingFeatures: "Fonctionnalités à Venir",
    userActivity: "Activité Utilisateur",
    verification: "Vérification",
    viewDetails: "Voir Détails",
    walletAddress: "Adresse Portefeuille",
    weeklyReport: "Rapport Hebdomadaire",
    withdrawalError: "Erreur Retrait",
    yearToDate: "Année à Date",
    prizeClaimInstruction: "Réclamez votre prix",
    
    // Referral Features
    referralSystem: "Système de Parrainage",
    myReferralCode: "Mon Code de Parrainage",
    referralEarnings: "Gains de Parrainage",
    depositBonus: "Bonus 100₪ quand l'invité fait premier dépôt 1000₪+",
    createQRCode: "Créer Code QR",
    shareMyReferralLink: "Partager Mon Lien de Parrainage",
    
    // Missing translations for NumberSelection
    selectYourLuckyNumbers: "Sélectionnez Vos Numéros Porte-bonheur",
    chooseNumbers: "Choisissez 6 numéros de 1 à 37",
    yourSelectedNumbers: "Vos Numéros Sélectionnés",
    selected: "Sélectionné",
    costPerTicket: "Coût par Ticket",
    purchaseTicket: "Acheter Ticket",
    previousNumbers: "Numéros Précédents",
    manualSelection: "Sélection Manuelle",
    fromHistory: "Depuis Historique",
    
    // User Level Progression
    currentLevel: "Niveau Actuel",
    progressToNext: "Progression vers Suivant",
    participationsLeft: "participations nécessaires",
    levelUpgrade: "Mise à Niveau!",
    
    // Dashboard Actions
    dashboardActions: "Actions Tableau de Bord",
    addFunds: "Ajouter Fonds",
    myHistory: "Mon Historique",
    supportChat: "Chat Support",
    lastDrawResults: "Résultats Dernier Tirage",
    draw: "Tirage",
    matches: "correspondances",
    winners: "gagnants",
    each: "chacun",
    myActiveTickets: "Mes Tickets Actifs",
    
    // Previous Numbers Selector
    hotNumbers: "Numéros Chauds",
    coldNumbers: "Numéros Froids",
    useTheseNumbers: "Utiliser Ces Numéros",
    recentDraws: "Tirages Récents",
    
    // Missing Landing Page translations
    clientLogin: "Connexion Client",
    backToHome: "Retour Accueil",
    clientAccess: "Accès Client",
    welcomeToLottery: "Bienvenue au Loto",
    register: "S'inscrire",
    phoneNumber: "Numéro Téléphone",
    confirmPassword: "Confirmer Mot de Passe",
    createAccount: "Créer Compte",
    
    // Missing Home Page translations
    lastUpdated: "Dernière Mise à Jour",
    reuseNumbers: "Réutiliser Numéros",
    participationAmount: "Montant Participation",
    participationLocked: "Participation Verrouillée",
    nextDrawInfo: "Info Prochain Tirage",
    drawCountdown: "Compte à Rebours Tirage",
    standardLotterySuggestion: "Suggestion Loto Standard",
    
    // Missing Admin translations
    drawCreated: "Tirage créé avec succès",
    resultsSubmitted: "Résultats soumis avec succès",
    createUserButton: "Créer Utilisateur",
    drawDate: "Date Tirage",
    
    // New MVP Features
    chatSupport: "Support Chat",
    referFriends: "Parrainer Amis",
    referralDescription: "Invitez des amis et gagnez 100₪ quand ils font leur premier dépôt de 1000₪+",
    progressToBonus: "Progression vers Bonus",
    referralsCount: "Nombre de Parrainages",
    won: "Gagné",
    correct_numbers: "Numéros Corrects",
    scan_qr_referral: "Scanner QR pour rejoindre avec votre lien de parrainage",
    draw_locked: "Tirage Verrouillé",
    participation_locked: "Nouvelle participation verrouillée 60 secondes avant tirage",
    standard_lottery: "Loto Standard",
    join_standard_lottery: "Rejoindre Loto Standard",
    jackpot_updated: "Jackpot Mis à Jour",
    last_update: "Dernière Mise à Jour",
    auto_update_in: "Mise à jour auto dans",
    
    // Landing Page French
    landingWelcome: "Bienvenue dans l'Expérience Ultime de Loto",
    landingSubtitle: "Rejoignez des milliers de joueurs dans le monde",
    currentJackpotLabel: "Jackpot Actuel",
    nextDrawLabel: "Prochain Tirage",
    drawsEveryWeek: "Tirages Chaque Semaine",
    fridayAt8PM: "Vendredi à 20h00",
    joinNow: "Rejoindre Maintenant",
    loginAsAdmin: "Accès Administrateur",
    demoAccess: "Accès Démonstration",
    chooseYourRole: "Choisissez Votre Accès",
    
    // Features French
    instantPayouts: "Paiements Instantanés",
    multiLanguage: "Multi-Langue",
    
    // Stats French
    totalPlayers: "Total Joueurs",
    weeklyJackpot: "Jackpot Hebdomadaire",
    winnersToDate: "Gagnants à ce Jour",
    
    // Auth Forms French
    enterCredentials: "Entrez Vos Identifiants",
    emailAddress: "Adresse Email",
    passwordField: "Mot de Passe",
    loginButton: "Connexion",
    registerButton: "S'inscrire",
    alreadyHaveAccount: "Vous avez déjà un compte?",
    dontHaveAccount: "Vous n'avez pas de compte?",
    
    // Admin Interface French
    adminDashboard: "Tableau de Bord Admin",
    managePlatform: "Gérer les Opérations de la Plateforme",
    totalTickets: "Total Tickets",
    totalRevenue: "Revenus Totaux",
    activeUsers: "Utilisateurs Actifs",
    completedDraws: "Tirages Terminés",
    
    // Demo Messages French
    demoMode: "Mode Démonstration",
    demoModeDescription: "Ceci est une démonstration avec des données d'exemple",
    
    // Error Messages French
    loginFailed: "Échec de la connexion. Veuillez réessayer.",
    networkError: "Erreur réseau. Veuillez vérifier votre connexion.",
    unexpectedError: "Une erreur inattendue s'est produite.",
    
    // Success Messages French
    loginSuccess: "Connexion réussie",
    accountCreated: "Compte créé avec succès",
    passwordChanged: "Mot de passe modifié avec succès",
    
    // Navigation French
    backToLogin: "Retour à la Connexion",
    goToHome: "Aller à l'Accueil",
    viewProfile: "Voir le Profil",
    accountSettings: "Paramètres du Compte",
    
    // Admin Dashboard Missing Keys French
    jackpot: "Jackpot",
    prizePool: "Cagnotte",
    revenue: "Revenus",
    ticketSales: "Ventes de Tickets",
    totalWinners: "Total Gagnants",
    submitDrawResults: "Soumettre Résultats du Tirage",
    date: "Date",
    selectWinningNumbers: "Sélectionner Numéros Gagnants",
    winningNumbers: "Numéros Gagnants",
    processing: "En cours",
    submitResults: "Soumettre Résultats",
    drawStatistics: "Statistiques du Tirage",
    ticketsText: "tickets",
    prizeDistribution: "Distribution des Prix",
    platformRetention: "Rétention Plateforme",
    user: "Utilisateur",
    drawCreatedSuccessfully: "Tirage créé avec succès",
    selectDrawDate: "Sélectionner date du tirage",
    createNewDraw: "Créer Nouveau Tirage",
    creating: "Création",
    createDraw: "Créer Tirage",
    platformRevenue: "Revenus Plateforme",
    retentionFromDraw: "Rétention du tirage",
    depositSuccessful: "Dépôt réussi",
    invalidAmount: "Montant invalide",
    searchUserByEmail: "Chercher utilisateur par email",
    userNotFound: "Utilisateur non trouvé",
    depositAmount: "Montant du Dépôt",
    enterAmount: "Entrer le montant",
    enterComment: "Entrer commentaire (optionnel)",
    depositing: "Dépôt en cours",
    yourWinningNumbers: "Vos Numéros Gagnants",
    noCompletedDraws: "Aucun tirage terminé trouvé",
    
    // ===== FINAL MISSING FRENCH TRANSLATIONS FOR 100% COVERAGE =====
    dashboardActions: "Actions Tableau de Bord",
    learningCenter: "Centre d'Apprentissage",
    lockedDraw: "Tirage Verrouillé",
    loginHistory: "Historique Connexions",
    loginToAccount: "Se Connecter au Compte",
    manualReview: "Révision Manuelle",
    maximumWithdrawal: "Retrait Maximum",
    minimumWithdrawal: "Retrait Minimum",
    moderationTools: "Outils de Modération",
    monitoringDashboard: "Tableau de Surveillance",
    myDashboard: "Mon Tableau de Bord",
    myTransactions: "Mes Transactions",
    noAccount: "Pas de Compte",
    notificationCenter: "Centre de Notifications",
    onboardingGuide: "Guide d'Intégration",
    operationalDashboard: "Tableau Opérationnel",
    or: "ou",
    paymentProcessor: "Processeur de Paiement",
    permissionControl: "Contrôle des Permissions",
    playerStatistics: "Statistiques Joueurs",
    qualityAssurance: "Assurance Qualité",
    reconciliationReport: "Rapport de Réconciliation",
    recurringPayment: "Paiement Récurrent",
    referralTracking: "Suivi de Parrainage",
    reportDownload: "Téléchargement Rapport",
    revenueStream: "Flux de Revenus",
    riskManagement: "Gestion des Risques",
    schedulePayment: "Programmer Paiement",
    securityProtocol: "Protocole de Sécurité",
    serviceStatus: "Statut du Service",
    stakeholderReport: "Rapport des Parties Prenantes",
    systemAlert: "Alerte Système",
    timelineView: "Vue Chronologique",
    transactionFee: "Frais de Transaction",
    userEngagement: "Engagement Utilisateur",
    validationRule: "Règle de Validation",
    workflowAutomation: "Automatisation du Flux",
    jackpotNotifications: "Notifications Jackpot",
    kycVerification: "Vérification KYC",
    loyaltyProgram: "Programme de Fidélité",
    mobileApp: "Application Mobile",
    multiCurrency: "Multi-Devises",
    onlineStatus: "Statut En Ligne",
    accessibilitySettings: "Paramètres Accessibilité",
    accountActivation: "Activation Compte",
    accountIssues: "Problèmes de Compte",
    accountLockout: "Verrouillage Compte",
    accountSettingsMenu: "Menu Paramètres Compte",
    accountVerification: "Vérification Compte",
    activeDraw: "Tirage Actif",
    activeStatus: "Statut Actif",
    advancedStats: "Statistiques Avancées",
    alertsManagement: "Gestion Alertes",
    apiManagement: "Gestion API",
    auditLogs: "Journaux d'Audit",
    autoResponder: "Répondeur Automatique",
    autoVerification: "Vérification Automatique",
    bannedStatus: "Statut Banni",
    betLimits: "Limites de Paris",
    bounceRate: "Taux de Rebond",
    browserNotifications: "Notifications Navigateur",
    bulkEmail: "Email en Masse",
    bulkSMS: "SMS en Masse",
    cancelledDraw: "Tirage Annulé",
    cancelledTransaction: "Transaction Annulée",
    chargebackProtection: "Protection Rétrofacturation",
    churnAnalysis: "Analyse d'Attrition",
    claimBonus: "Réclamer Bonus",
    clickRate: "Taux de Clic",
    cohortAnalysis: "Analyse de Cohorte",
    complaintSubmission: "Soumission Plainte",
    completedDraw: "Tirage Terminé",
    completedTransaction: "Transaction Terminée",
    confirmedCrypto: "Crypto Confirmé",
    contactInfo: "Informations Contact",
    contentManagement: "Gestion Contenu",
    conversionAnalytics: "Analytique Conversion",
    cooloffPeriod: "Période de Refroidissement",
    cryptoDeposit: "Dépôt Crypto",
    cryptoFees: "Frais Crypto",
    cryptoHistory: "Historique Crypto",
    cryptoSettings: "Paramètres Crypto",
    cryptoSupport: "Support Crypto",
    cryptoWallet: "Portefeuille Crypto",
    cryptoWithdrawal: "Retrait Crypto",
    currencySettings: "Paramètres Devise",
    customLimits: "Limites Personnalisées",
    customerSupportMain: "Support Client Principal",
    deviceManagement: "Gestion Appareils",
    displaySettings: "Paramètres Affichage",
    emailConfig: "Configuration Email",
    emailDelivery: "Livraison Email",
    emailHistory: "Historique Email",
    emailQueue: "File d'Attente Email",
    emailSupport: "Support Email",
    emailSystem: "Système Email",
    emailValidation: "Validation Email",
    errorLogs: "Journaux d'Erreur",
    eventTracking: "Suivi d'Événements",
    excludedUser: "Utilisateur Exclu",
    expiredDraw: "Tirage Expiré",
    exportData: "Exporter Données",
    faqSection: "Section FAQ",
    feedbackForm: "Formulaire Commentaires",
    fraudDetection: "Détection Fraude",
    gameConfiguration: "Configuration Jeu",
    gameIssues: "Problèmes de Jeu",
    gameSettings: "Paramètres Jeu",
    generalInquiry: "Demande Générale",
    generalSettings: "Paramètres Généraux",
    geoBlocking: "Blocage Géographique",
    helpCenterMain: "Centre d'Aide Principal",
    historyTracking: "Suivi Historique",
    inAppNotifications: "Notifications In-App",
    incompleteTransaction: "Transaction Incomplète",
    ipRestrictions: "Restrictions IP",
    knowledgeBase: "Base de Connaissances",
    languageSettings: "Paramètres Langue",
    liveChatSupport: "Support Chat en Direct",
    loginAttempts: "Tentatives Connexion",
    lotteryOperations: "Opérations Loterie",
    maintenanceNotification: "Notification Maintenance",
    marketingCampaigns: "Campagnes Marketing",
    multipleDevices: "Appareils Multiples",
    networkIssues: "Problèmes Réseau",
    notificationChannels: "Canaux Notification",
    operationalLogs: "Journaux Opérationnels",
    passwordPolicy: "Politique Mot de Passe",
    paymentIssues: "Problèmes Paiement",
    personalizedOffers: "Offres Personnalisées",
    phoneSupport: "Support Téléphonique",
    playTime: "Temps de Jeu",
    prioritySupportService: "Service Support Prioritaire",
    privacySettings: "Paramètres Confidentialité",
    pushNotifications: "Notifications Push",
    realityCheck: "Vérification Réalité",
    responsibleGaming: "Jeu Responsable",
    riskAssessment: "Évaluation Risques",
    searchHelp: "Recherche Aide",
    securityConfiguration: "Configuration Sécurité",
    securityQuestions: "Questions Sécurité",
    selfExclusion: "Auto-Exclusion",
    sessionTimeout: "Délai Session",
    smsAlerts: "Alertes SMS",
    spendingLimits: "Limites Dépenses",
    stepByStepGuides: "Guides Étape par Étape",
    suggestionBox: "Boîte à Suggestions",
    supportHours: "Heures Support",
    supportLanguages: "Langues Support",
    suspendedUser: "Utilisateur Suspendu",
    systemHealth: "Santé Système",
    telegramNotifications: "Notifications Telegram",
    themeSettings: "Paramètres Thème",
    ticketSupport: "Support Ticket",
    timezoneSettings: "Paramètres Fuseau Horaire",
    transactionLimits: "Limites Transaction",
    troubleshooting: "Dépannage",
    tutorials: "Tutoriels",
    twoFactorAuthentication: "Authentification Deux Facteurs",
    videoGuides: "Guides Vidéo",
    verificationRequired: "Vérification Requise",
    whatsappNotifications: "Notifications WhatsApp",
    withdrawalLimits: "Limites Retrait",
    requestTypes: "Types de Demande",
    technicalSupport: "Support Technique",
    
    // ===== FINAL ADDITIONAL FRENCH TRANSLATIONS FOR 100% COVERAGE =====
    notificationTypes: "Types de Notification",
    openRate: "Taux d'Ouverture",
    passwordChange: "Changement Mot de Passe",
    paymentNotifications: "Notifications Paiement",
    paymentProcess: "Processus Paiement",
    paymentProcessing: "Traitement Paiement",
    paymentSecurity: "Sécurité Paiement",
    pendingCrypto: "Crypto en Attente",
    pendingWithdrawals: "Retraits en Attente",
    performanceMetrics: "Métriques Performance",
    personalizedSupport: "Support Personnalisé",
    platformAnalytics: "Analytique Plateforme",
    platformMaintenance: "Maintenance Plateforme",
    playerRewards: "Récompenses Joueur",
    priorityAccess: "Accès Prioritaire",
    quickWithdrawal: "Retrait Rapide",
    realTimeSupport: "Support Temps Réel",
    recentActivity: "Activité Récente",
    referralBonus: "Bonus Parrainage",
    registerHere: "S'inscrire Ici",
    reportGeneration: "Génération Rapport",
    revenueTracking: "Suivi Revenus",
    riskMonitoring: "Surveillance Risques",
    secureAndTrusted: "Sûr et Fiable",
    securityEvents: "Événements Sécurité",
    signIn: "Se Connecter",
    smartphoneApp: "Application Smartphone",
    socialSharing: "Partage Social",
    specialOffers: "Offres Spéciales",
    systemOverview: "Vue d'Ensemble Système",
    thirdPartyIntegrations: "Intégrations Tierces",
    transactionAnalytics: "Analytique Transaction",
    transactionHistory: "Historique Transaction",
    userAcquisition: "Acquisition Utilisateur",
    userFeedback: "Retour Utilisateur",
    userPreferences: "Préférences Utilisateur",
    userRetention: "Rétention Utilisateur",
    userSettings: "Paramètres Utilisateur",
    versionUpdate: "Mise à Jour Version",
    webNotifications: "Notifications Web",
    welcomePackage: "Package Bienvenue",
    winNotifications: "Notifications Gain"
  }
};

export type Language = keyof typeof translations;
export type TranslationKey = keyof typeof translations.en;

export const getTranslation = (key: TranslationKey, language: Language): string => {
  return (translations[language] as any)[key] || translations.en[key] || key;
};

export const formatAmount = (amount: number, language: Language): string => {
  const currency = translations[language].currency;
  return language === 'he' ? `${currency}${amount.toLocaleString()}` : `${currency}${amount.toLocaleString()}`;
};

export const formatJackpot = (amount: number, language: Language): string => {
  const currency = translations[language].currency;
  return language === 'he' ? `${currency}${amount.toLocaleString()}` : `${currency}${amount.toLocaleString()}`;
};